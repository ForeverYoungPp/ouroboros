# Ouroboros v6.114.42 — Architecture & Reference
...
server.py (Starlette+uvicorn) ← HTTP + WebSocket on configurable host:port (default localhost:8765; Docker/non-loopback supported via OUROBOROS_SERVER_HOST=0.0.0.0)
  ...
  │   ├── task_lifecycle.py    ← Queue-owned root-budget admission fence and subtree-cancellation fencing, and cancellation custody (the ONE settle owner of durable cancel intents: claim/generation fence, capture → confirmed death → natural-completion re-check → artifact capture → settled write, cascade postcondition, watchdog replay; full invariant in §5 Supervisor Loop). Extends queue state without a second lifecycle authority |
  ...
  │   ├── queue_transitions.py ← Queue-owned lifecycle TRANSITIONS not cancellation custody: acceptance-fence open/inspect/seal, explicit budget resume, the typed evolution stop (durable-intent + custody ingress, never an in-place prune), and fenced Project deletion (cascade lineage ROOTS, tombstone only after provable quiescence). One-way dependency; `supervisor.queue` re-exports these names |
  │   ├── terminal_delivery.py ← (Poltergeist A2) Durable terminal-answer delivery seam: restart-surviving `delivery_id` dedupe + bounded PENDING outbox (`state/terminal_deliveries.json` — owed-before-enqueue, replayed on boot/tick, capacity/exhaustion disclosed loudly, strict row validation). Shared by natural final-answer, cancel salvage, cascade digest, and non-retry reap; full invariant in §5 Supervisor Loop |
  │   ├── task_reaper.py       ← (v6.38.0) Variant A off-loop worker reaper: kill/join/archive/respawn a timed-out worker off the loop critical path. STRICT fail-closed: an unconfirmed-dead worker holds the slot `reaping` (no terminal/task_done/retry/respawn while alive, wedged receipt + restart hint). After confirmed death and BEFORE retry/respawn it reconciles open DELEGATED runs; it never mints cancel intents |
  │   ├── owner_stop.py        ← (S3, Q1) Owner graceful-stop episode: `finalize_then_cancel` policy for `POST /api/tasks/{id}/cancel` — the SAME durable cancel intent (policy monotonic: immediate HARDENS a pending graceful, never softens). One bounded `finalize_now` control whose grace budget starts only when the loop drains it, under an outer `OWNER_STOP_OUTER_CAP_SEC` cap; a COMPLETED finalize root suppresses the redundant cascade summary. Full invariant in §5 |
  ...
  │   ├── events.py            ← Event dispatcher (worker→supervisor events): exact task-attempt/execution/round/call correlation for the process-local active main-LLM row, managed-update assisted-merge orphan watchdog hook, and the frozen subagent task text composer. Unregistered event types are dropped into a truncated `unknown_worker_event` repr; `tests/test_worker_event_registry.py` pins the class by AST scan (every literal `{"type": ...}` dict to a `.put`/`pending_events.append`/`emit_review_event` wrapper is registered or allowlisted). Pure admitted-event→worker-payload normalization lives in `task_dispatch.py` |
  ...
  └── ouroboros/               ← Agent core (runs inside worker processes)
      ...
      ├── agent_task_pipeline.py  ← Task execution pipeline orchestration; emits a per-task `swarm_efficiency` rollup (subagent_count/wave_count/Σ inter-wave latency/lanes_requested — effective lanes are per-child dispatch facts, never inferred from pre-dispatch fanout events) plus a `no_fanout_observed` block for Swarm-intent tasks that spawned nothing; freezes one shared non-final subtree-cost snapshot for summary/reflection before the terminal checkpoint |
      ...
      ├── engram_client.py     ← Standalone Engram HTTP client (seed C1: HTTP only, never `mcp_client.py`). One repo-pinned project scope resolved WITHOUT cwd detection (cwd would scatter one system's memories across several project names); a blank/`local`/forbidden name is a config error, never a silent fallback. Transport failures are typed data (`transport|timeout|http|decode|config`), never exceptions, and every limit is clamped client-side (search ≤20, timeline radius ≤10, hard ceiling 500, `max_bytes` ≤65536) because an unbounded pull is indistinguishable from a correct one until the context bill arrives. `env_repo_root()` gives callers that hold only the drive root the same project anchor
      ├── engram_sink.py       ← The Engram WRITE path: gate → bound → **spool** → send, one `emit` per confirmed source (`memory_action` | `knowledge` | `dialogue_summary` | `continuation_narrative` | `evolution_checkpoint` | `evolution_cycle_outcome` | `review_verdict`). C18 gate first: content whose JOB is to say what to do next is refused (`todo_not_memory`) and to-do-only field payloads are rejected, so a work item cannot become a memory by accident. C17 durability: the record is appended to `state/engram_spool.jsonl` BEFORE the network call and acknowledged after, so an outage or a crash costs a retry and never the record; the spool is append-only, bounded (`SPOOL_MAX_ENTRIES`), over-bound sends are refused with a typed reason, and a torn tail is skipped rather than fatal. Identity is the upsert key (topic / task_id / the INTERVAL a dialogue block covers), which is what makes a forward retry idempotent across processes. `fields` are NOT transmitted (the server has no metadata column) — anything a reader must find remotely has to be in the title, the content, or the identity. Unreachability is disclosed once per run to its own `logs/engram.jsonl`, never into the canonically-ordered `logs/events.jsonl`
      ├── engram_read.py       ← The Engram READ path: bounded, typed machine reads. Five outcomes stay separate — `ok` / `empty` (the store answered and holds nothing) / `rejected` (the store ANSWERED and refused the request — an unknown project, a missing session, an invalid selector — or the client refused to send it because the scope is unresolved) / `unavailable` (the store could not be read) / `stale` (content served from `engram_cache` because the store could not be RE-read, carrying the failed read's own cause) — because collapsing them is how a memory-loss signal becomes noise. Hosts `digest`, `type_digest` (Engram exposes no type-filtered recency read, so the filter is applied over a bounded `MAX_WINDOW`) with an explicit saturation disclosure, `due_for_review` (consciousness's native review cursor), `knowledge_topic` and `continuation_narrative` (single-record reads by the identity the sink wrote). `digest`/`recent` are deliberately windowed well below Engram's 500 ceiling: that route has no compact mode, so its ceiling means "the whole store's bodies in one request"
      ├── engram_cache.py      ← Read-through cache for prompt-side Engram sections: TTL-bounded (`DEFAULT_TTL_SECONDS`), keyed by (kind, resolved project, resolved drive, variant) so two projects — or two drives, since the in-process cache outlives one drive — can never share an entry. When the store cannot be re-read it degrades to the LAST GOOD read with an explicit `stale` status rather than a blank that reads as "there is nothing", and a short failure backoff (the failed attempt's own timestamp) stops a wedged store from being re-probed on every turn at the cost of the full transport timeout. Query-keyed entries are in-process only, while stable keys are mirrored under `state/engram_cache/`; both caches are bounded (`MAX_CACHE_ENTRIES`, `MAX_CACHE_FILES`, `MAX_CACHE_CHARS`), the disk mirror pruned newest-first. It deliberately does NOT cache the review cursor — that read authorizes a write (`mark_reviewed`), so a stale cursor could advance the decay clock on a moved set — nor task-keyed reads, where a later turn asks a different key and an entry could never be asked for twice
      ├── engram_conflicts.py  ← Conservative conflict policy over Engram's NATIVE loop (`GET /conflicts`, `POST /conflicts/compare`, `/judge`, `/scan`). `decide_verdict` is a pure function: only `not_conflict` lands unattended (it asserts the two records are about different things, can rewrite neither, and is what suppresses future candidate scans); `conflicts_with`/`supersedes` are findings for the owner, never an unattended licence to edit history; everything else is surfaced too. The module has no delete path and never emits `supersedes`, so both sides of a conflict survive by construction rather than by discipline. A `not_conflict` the store accepted without returning a relation id is reported as FAILED — the suppression did not happen, and the caller must not be told that it did
      ...
      ├── loop.py              ← High-level LLM tool loop; finalization nudges (no-op / red-verification / masked-verification / nanny), continuous `FINAL ANSWER:` latching, answer-protocol gating, and forced-finalization rails. Full invariant in §6 Task lifecycle |
      ...
      ├── task_pacing.py       ← Task-pacing SSOT: deadline/cost milestones, finalization reserve, BudgetSnapshot, and acceptance-review launch/improvement rails (first review reserves ≥200s, later `max(floor, 1.5×EWMA)`); explicit `max_improvement_passes` binds, else the shared `OUROBOROS_REVIEW_MAX_CYCLES` cap. Legacy `until_deadline`/`stall_rounds_threshold` accepted for one compat window. v6.74.4: workspace deliveries get one shared commit-neutral tree sentence on deadline/cost milestones (commit-neutral because acting children cannot commit). Disclosed residual: a forced tool-less exit inside one long round can still ship an unverified last edit — structural seam is owner-pending |
      ...
      ├── cancel_intents.py    ← (Poltergeist A1) Durable cancel-intent projection: compact locked `state/cancel_intents.json` of ACTIVE intents + forensic `cancel_intent` rows; ONE ingress `request_cancel` (agent tool, HTTP single/cascade, boot migration); STRICT read (corrupt file fails closed, never {}-collapse); row-level strictness (GR6-3); `allow_settled_target` is the live-ownership exception (GR6-1). Full invariant in §5 Supervisor Loop |
      ├── owner_hurry.py       ← (S3, HQ1) Owner "hurry" control — a typed, TASK-LOCAL acceleration latch, never a chat message: durable `owner_hurry` projection keyed by `task["_attempt"]` (writer = `update_json_locked` ONLY on that key), worker-drained attempt-local latch (skip next acceptance panel, zero improvement passes, force-plan advisory). Effect DIES WITH THE ATTEMPT (`retry_reset` on every same-id requeue). P3/commit/review gates untouched; non-chat `owner_hurry` events |
      ...
      ├── _outcome_receipts.py ← Private pure helpers for parsing append-only verification receipts, the ONE canonical receipt IDENTITY derivation (`receipt_canonical_identity` — criterion_id / canonical check text paired with its rendering / canonical paths set; keyed sameness fails SAFE, no fallback chain), the outstanding red/masked SETS (all-later-reconciler scan), the shared disclosed identity/list projections (bounded lists disclose omitted count + hash), and superseded acceptance-run reconciliation. Full invariant in `ouroboros/_outcome_receipts.py` + DEVELOPMENT.md §Anti-pattern |
      ...
      ├── delegate_custody.py ← Durable custody for delegated (Claudexor) runs: SSOT is `delegate_run_*` rows in the canonical event log + ONE compact projection `<drive_root>/logs/containment_faults.jsonl` (so an unresolved fault cannot fall out of health invariants); OWNED/FOREIGN/UNKNOWN ownership surviving restart, per-intention idempotency-key invocation ids, typed cancel vocabulary with durable containment faults, orphan reconciliation on the owner-is-gone predicate, `daemon_says_absent` scoping, and the patch-apply-intent protocol. Full invariant in §6 Delegated subagents |
      ...
      ├── delegate_evidence.py ← Read-side execution-evidence projection over the custody rows (`task_execution_evidence`: started/settled/succeeded/failed, `evidence_read_failed`, disclosed spend, `nanny_nudge_recorded`, `delegate_start_attempted`); owns the nudge/attempt writers; projects `applied_access_profiles` (D29, from SETTLED rows only) and `acceptance_patch_dispositions` (cap 20 + exact omitted count; absence = no disposition recorded, never clean). Full contract in §6 Delegated subagents |
      ...
      ├── reasoning_artifacts.py ← Shape-first classification of REPLAYED OpenRouter reasoning artifacts as sealed (endpoint-bound: encrypted, signed, redacted, or a shape it cannot read — fail-closed) versus portable (readable text/summary, plus every form for a vouched family), consumed by both `llm.py` consumers of that one truth: the proactive dispatch continuity pin and the reactive same-model reroute's preserve-vs-strip choice. The `SIGNED_PORTABLE` roster is a MUTABLE EXTERNAL PROVIDER FACT (see the decaying-facts inventory in `docs/DEVELOPMENT.md`), extended only by a fresh cross-provider replay probe of the exact family
      ...
      ├── safety.py            ← Policy-based LLM safety check: context bounded by `_SAFETY_CONTEXT_CHAR_BUDGET` (newest-first, omission marker reserved INSIDE the budget); a rate limit on the check itself takes one bounded deadline-capped backoff + one retry then BLOCKS with the typed non-verdict `⚠️ SAFETY_UNAVAILABLE` (plain tool error, never `safety_violation`) + durable `safety_check_rate_limited` audit — a 429 is an infrastructure fact, the agent retries the same call; `full` keeps the owner contract that an unchecked guarded call never executes (fail-open = exactly the three no-backend cases); a process-local storm latch answers the window without new provider calls |
      ...
      ├── task_tree_ledger.py  ← (v6.38.0) Task-tree coordination ledger keyed by `root_task_id` — the domain-agnostic swarm blackboard + typed child→parent beacons. Append-only `data/task_trees/<root>/blackboard.jsonl` (size-capped, GC-eligible with the tree); kinds: coordination (contract/decision/fact/note) + beacon (milestone/partial_finding/blocker/question/interface_contract/review_requested/delegation_constraint). `review_requested` is advisory non-blocking (exact evidence ref + SHA-256, no reviewer starts automatically); `delegation_constraint` rows carry structured payloads (non-advisory rows gate admission until a later decision overrides; prose never authoritative). Exposed via `tree_note`/`tree_read`; tail injected each turn; aged out by `headless.prune_task_trees`; high-signal rows mirrored into the durable project journal at root terminal |
      ├── projects_registry.py ← Multi-project registry: durable `data/state/projects.json` with immutable id/chat identity, 80-char display names, working-folder facts, `active|deleting|tombstoned` lifecycle, routing fence/generation, visible revision; deletion preserves bindings/history/folder/memory and an id never resurrects while its tombstone row lives (disclosed residual: losing projects.json loses rows AND tombstones; preserved task binding then vouches for recovery). Boot/tick reconcile registers an existing store without a row and NEVER prunes; workspace-derived proj_ stores register only on proof of a real room (durable task binding, or the registry-authored `.project.json` provenance echo); a ghost row can never mint the marker for itself |
      ...
      ├── capability_evidence.py ← Sourced, route-fingerprinted context-window EVIDENCE (v6.33.0): provider `/models` metadata, local n_ctx, or owner acknowledgement; each claim carries `confirmed`/`asserted`/`unprobeable`/`failed`, persisted to `data/state/capability_evidence.json`; consulted by the ≥1M scope-reviewer floor and Main route-window fit (W). Also owns `observe_token_density` (with the additive `basis` field — `"raw"` vs `"bounded_proxy"` — so measurement bases are never silently mixed; budget reservation deliberately keeps RAW for money-safety) |
      ...
      ├── delegate_interactions.py ← Interactive-question cluster for delegated runs: reported-question memo (`_REPORTED_INTERACTIONS`), bounded inline projection (all harness-authored DISPLAY scalars bounded; answer keys ride whole), immediate typed `waiting_on_user` payload (full set spills whole to task drive with sha256 receipt), and `_delegate_answer` (strict pre-POST row validation, engine-typed outcomes relayed verbatim; `subscription_window_exhausted` distinct from payload-semantic `rejected`; `delivery_unknown` for transport death). Full contract in §6 Delegated subagents |
      ...
      ├── subagent_bootstrap.py ← Pre-first-round bootstrap for configured session nannies (charter, 2026-08-28/29): freeze immutable route/work-order authority, then START the exact snapshotted leaf BEFORE the first metered model round (one start path, one refusal shape set). Branch order: recovery adoption → durable zero-run/unknown-evidence fences → blocked dispatch → pre-start. Host never waits; a blocked route or DEFINITE refusal ends unrun at $0; anything ambiguous wakes the model. Full contract in §6 Delegated subagents |
      ...
      ├── delegate_terminal.py ← Task-scoped delegated-run reconciliation and terminal custody audit persistence; write-side refresh of a TERMINAL task's stored custody disclosure (sweep refresh, once-per-generation boot backfill, kill-path clear) — change-gated (unchanged audit = no write/emit); run counters stay a historical snapshot (owner Q2=B), `delegate_terminal_reconciliation` envelope is current-liveness. `refresh_recently_settled_terminals` covers stale-counter rows via a durable byte-offset cursor. Full contract in §6 Delegated subagents |
      ...
      ├── subagent_worktrees.py ← Acting self_worktree lifecycle: provision/remove/prune isolated git worktrees (outside repo/data) + durable registry (state/subagent_worktrees.json) + cross-process ops lock; startup orphan reconciliation; provisions durable from-scratch genesis projects (provision_genesis_project, never registry/GC); owns the C1 delegated-exec snapshot lifecycle (`provision_execution_snapshot` synthetic baseline pinned by `refs/ouroboros/delegated/`, detached private worktree) and the standalone payload snapshot (non-Git skill payload, loader-inventory copied, CAS-baseline hash, writer-race abort on mismatch). Removal only by explicit disposition or custody-cross-checked startup GC |
      ├── artifacts.py         ← Task-scoped artifact helpers shared by user-file tools, process outputs, chat-media replay, and outcome finalization: `stage_task_attachments` (input attachments → `artifact_store/attachments/`, secret-source skipped), `collect_task_artifact_records` (excludes input/chat_media transport bytes), `record_task_scratch`/scratch fingerprints (ephemeral verification files excluded from workspace patch only while content matches; undeclared-output guard stat-verifies POST-exec), and `delegated_capture_read_target` (narrow artifact_store READ rebinding for split-drive nannies). Full contract in §6 + DEVELOPMENT.md |
      ...
      ├── preflight_runner.py  ← Hermetic reviewed-change pytest gate: disposable git worktree, ONE hardened candidate capture (`git diff --binary … HEAD` applied as RAW BYTES for every index state incl. unfinished merge; capture/apply failure = typed hard block PREFLIGHT_CANDIDATE_ASSEMBLY, never a test verdict), temp data/settings/pycache env, live OUROBOROS_*/secret-class scrub. Runs the node web-tests lane then CI's own two-pass split (parallel `not serial` then flag-free `serial`) under ONE budget with `LANE_EXCLUSION_EXPR` as marker-lane SSOT; dead xdist worker / missing plugin are distinct named hard blocks, never silent fallback |
      ...
      ├── review_substrate.py  ← Reviewer-slot coordinator used by task acceptance and planning helpers; duplicate model ids remain independent slots. ONE seam: `_run_slot` binds an immutable `ReviewAssignment` once through `review_execution._review_route_executor` (closed `ReviewRouteKind`: `api_chat` | `agent_session`); per-actor transport/parse/semantic/coverage facts stay distinct, only a compact projection reaches UI. Task acceptance enforces adaptive quorum, one substantive call + ≤2 physical attempts, metric-grounded evidence. Full delivery contract in §6 Review stack |
      ...
      ├── review_execution.py  ← (v6.87.21, phase 5) Review execution BELOW the substrate's seam: closed route vocabulary (`ReviewRouteKind`: `api_chat` | `agent_session` — never a vendor/harness name), immutable `ReviewAssignment`, per-route executors returning typed `ReviewAttemptResult` (a route that cannot deliver raises `ReviewRouteUnavailable` on its own slot, never fallback), lazily-memoized api_chat prompt renderers (durable prompt record + both permitted sends share one byte-identical rendering), and `AgentSessionReviewExecutor` with the shared `run_delegated_review_session` nanny loop + D19 typed-verdict order. One-way dependency; `review_substrate` re-exports the historical renderer names |
      ├── review_native_episode.py ← Bounded native tool-round review executor (`NativeToolRoundReviewExecutor`) for api_chat reviewer rows that reference a configured subagent (and for the api_chat advisory): ONE logical review episode of at most `OUROBOROS_REVIEW_NATIVE_MAX_ROUNDS` `chat(tools=…)` calls against a fresh instance-local inspection registry (read_file/list_files/search_code/query_code/vcs_status/vcs_diff under a `local_readonly_subagent` constraint with network/web off); every physical send is paid-stamped and ledger-accounted inside the episode, tool receipts (≤200, path/root/query-shaped) and `host_file_read_attestation: host_observed` ride the actor usage, the transcript cap and an empty deliverable fail closed with typed refusals (`native_rounds_exhausted`, `native_transcript_cap_exceeded`), the LAST permitted round WITHDRAWS the tools (`_FORCED_FINAL_NOTICE`) so an episode that inspected to the cap still delivers the verdict it was paid for, that deliverable turn is floored at the review surface's own output budget (`native_episode_answer_budget` — hidden reasoning shares the output budget with the answer, so a slot-sized budget can be consumed entirely by reasoning and yield zero content, measured 2026-09-13), and the second actor attempt is a LOCAL format repair over the collected answer — never a second paid episode
      ...
      ├── review_slot_cancel.py ← Hosted-review slot poller's cancel-honesty helpers: early-termination for a parked question (`_interaction_outlives_slot`), verified slot cancel reporting only what it PROVED (`_slot_cancel_outcome`), honest attribution wording (`_cancel_honesty_clause` — "host-cancelled" only on a `confirmed` receipt whose state is the cancel's own), and completion-wins consumption of a discovered natural success (`_natural_success_terminal`, BR2-1/BR2-2). |
      ...
      ├── review_dispatch.py   ← Reviewer dispatch primitives: the row-identity mint (`slot_id_for_row` + surface prefixes) and the write-ahead PAID stamp seam — a metering gate installs an idempotent `ReviewPaidStamp` on `ctx._review_paid_stamp`; session routes invoke it before replayable `START_REQUESTED`, API routes bind it until the canonical attempt ledger durably reaches `dispatched` immediately before wire send (typed pre-start refusals stay $0; a late worker/crash cannot race away the durable paid fact). Task acceptance binds its exact-hash tree-wallet claim immediately before that API transition |
      ...
      ├── triad_review.py      ← Shared multi-model review primitives: JSON-array extraction (repo + skill review), per-actor records, quorum/degraded accounting, model-error events; owns the review OUTPUT CONTRACT text for repo-triad/advisory/scope prompts — findings-only (`REVIEW_JSON_ARRAY_CONTRACT`) and required-matrix (`REVIEW_JSON_MATRIX_CONTRACT`, no all-clear when `expected_items` supplied). A clean findings-only verdict is recognized only when the WHOLE response — modulo one optional code fence — is `[]` optionally followed by `NO_FINDINGS` and nothing else; surrounding prose or sentinel-without-array are parse failures. Kept beside `empty_array_is_verified_clean` (the enforcing parser) so the two cannot drift |
      ...
      ├── review_evidence.py   ← Builds bounded, provenance-tagged task-acceptance evidence from effective task/plan claims, verification support, artifacts, tool trajectory, obligations, retrieval facts; ingress claims win over the current closed plan wave (projected without mutating the live contract); structured summaries feed reflection. For harness-dispatched tasks the packet carries a host-attested `substrate_execution` section (VISIBILITY ONLY — acceptance judges quality, never execution route, owner 2026-08-28); sibling `delegated_patch_dispositions` carries parent's host-attested patch apply/reject attestations (owner 4=A) — ATTEST the apply, never invent a review; visibility, never a gate |
      ...
      ├── marketplace/         ← ClawHub + OuroborosHub marketplace package: clawhub.py registry client, ouroboroshub.py static GitHub catalog client (display-only TTL catalog memo, `run_hub_update`), and the §-frozen adopt transaction (eligibility by physical location, CAS with post-move-aside re-verification, state-quintet snapshot/rollback with VERIFIED `rolled_back` — restore failures disclosed as `rollback_errors`; depth-1 retained `.pre-adopt` copy with disclosed retention failure; abortive-review rows from a rolled-back adopt remain in the append-only `review_history.jsonl` — disclosed residual); fetcher.py staging, adapter.py OpenClaw→Ouroboros translation, install.py orchestration, isolated_deps.py per-skill dependency prefix, provenance.py durable provenance (ClawHub install record + locked merge-writer + OuroborosHub publication receipt `state/skills/<n>/ouroboroshub.json` at pr_opened) |
      ...
      ├── shell_parse.py       ← Shared shell argv/inline-command parser helpers used by guardrails without importing the tools package: `recover_stringified_argv` (SSOT JSON/AST stringified-argv recovery shared by run_command + verify_and_record), `normalize_check_argv` (verify check→argv SSOT the shell guard AND execution both call — guard inspects exactly what runs; string → non-login `sh -c`), `shell_tokens_typed` (tokens paired with real-SYNTAX vs literal-spelling classification; `_normalize_shell_source` marks quoted/escaped punctuation), `shell_tokens` text view (quoted `&&` still reads as separator — over-split is fail-safe), and `canonical_command_text` (comparison-stable form — one space BETWEEN tokens, contents/operators verbatim; the seam `_outcome_receipts` derives verification check identity from, so collapsing inside a quoted arg or stripping a literal `'&&'` can never make two checks compare equal) |
      ...
      │   └── claudexor.py     ← Claudexor v3 control-plane gateway (loopback discovery, protocol-major + minimum-version handshake, project registration, run start/poll/cancel, cached quota GET, and explicit foreground quota POST; the daemon token stays inside this module). (D30) Also the account-surface translations — credential profiles, harness model discovery, setup/login jobs with their transient device-code snapshots — all read/translate only: the daemon owns every auth fact. Default discovery prefers the OWNED daemon once provisioned (`claudexor_daemon.owned_daemon_provisioned`), an explicit `home=` still reads that home verbatim, and `discover_daemon_at` reads the `CLAUDEXOR_CONFIG_DIR` override layout (`<config_dir>/daemon/control-api.json`)
      ├── claudexor_runtime.py ← Exact managed engine/CLI delivery. One reviewed pin selects the public Node-free Claudexor closure by version/build SHA/URL/SHA-256/size/protocol/exact tested Node/daemon entrypoint + nullable CLI entrypoint (`null` = pre-CLI closure). New packages carry the compressed archive as seed; older apps download the same bytes. Both paths verify size+digest, extract to a private staging dir, run the side-effect-free daemon identity probe with host Node, and atomically promote ONE immutable version under the short `data/state/cx` root. Managed CLI stricter: requires the pin's POSIX Node archive supplying both `bin/node` and the npm subtree; never resolves PATH/system CLI/npm. No mutable `current` pointer or background updater: the code pin is next-spawn selection; a newer tree may stage beside a live older daemon |
      ├── claudexor_daemon.py  ← (D30) The Ouroboros-OWNED claudexord: data-plane config dir (`data/claudexor` as `CLAUDEXOR_CONFIG_DIR`), supervision after `process_custody.spawn_supervised`, ATTACH-IF-ALIVE / OWN-ONLY-IF-SELF-STARTED, provisioning as an OWNER ACTION, `ensure_owned_gateway` as the one install/probe/spawn seam, authenticated-liveness + stale-lifecycle reconcile, and bounded admission wait (`daemon_recovery_only`). Full lifecycle contract in §6 Delegated subagents |
      ...
      │   ├── claudexor_accounts.py ← (D30) Agent accounts HTTP surface (Settings → Agents → Accounts): six thin proxies of the owned daemon's account truth — GET /api/claudexor/status (catalog/accounts/quota facets + `unified_accounts` feature fact), POST /api/claudexor/wake, POST /api/claudexor/login (Connect intent + setup job create/cancel/input/reconcile on /api/claudexor/login/{job_id}), credential-profile delete/patch (DELETE /api/claudexor/credential-profiles/{harness}/{profile_id}). Zero auth logic here; full contract in §3 Settings/onboarding + `gateway/claudexor_accounts.py` |
      │   ├── claudexor_quota.py ← Explicit owner quota-refresh transport: POST /api/claudexor/quota/refresh discovers the already-owned daemon, performs the mandatory protocol handshake, and delegates exactly once to Claudexor POST /v2/quota. Handshake retains the ordinary 60-second control-plane read bound; only the foreground quota POST receives 90 seconds (the default plus one structural settlement margin) for sequential vendor work. It returns that foreground envelope verbatim; no lifecycle start, cached status composition, account/catalog read, quota policy, retry, or daemon token crosses this inbound boundary. GET /api/claudexor/status remains passive.
      ...
      │   ├── claude_advisory_review.py ← Preflight (advisory) pre-commit review tool — `preflight_review`, with the callable `advisory_review` compat alias. Admission policy (freshness/skip/obligations/debt) stays here; the critic delivery rides the shared review substrate: api_chat rows run the bounded NATIVE inspection episode (`review_native_episode.py`) on the configured routed model (same-model payable-spelling fallback for direct-key installs), agent_session rows ride `AgentSessionReviewExecutor`. Both deliveries ALSO retrieve their EVIDENCE (`evidence_by_retrieval`): the touched-file pack — full content of every changed file, ~940K-1.15M chars with no TOTAL budget — is replaced by a `vcs_diff`/`read_file` recipe, and only the bounded staged diff (the subject) stays inlined; inlining the pack overran the episode's transcript bound and then spent its rounds re-reading content the prompt already carried. The retired Claude-SDK transport has no code path left
      ...
      │   ├── verify.py          ← (v6.47.0) `verify_and_record` core tool: the HOST runs the declared verification `check` through the same PRE-EXECUTION machinery as run_command (shell-guard, path bootstrap, executor backend), then writes a durable host-attested receipt (`verification_receipts.jsonl`). Modes: `expected_match`, `artifact_paths` after-only lifecycle FLAG, exit-masking sensor, `delegation_zero_run` typed decision. Full contract in `ouroboros/tools/verify.py` |
      ...
      │   ├── delegate_integration.py ← The C1 integration seam of the nanny verbs (delegate.py re-exports it): `_mutation_authority` (target_root/source/capture_mode), `_provision_snapshot` (private execution snapshot registered BEFORE start intent), retry binding (ONE durable invocation record re-proven), `_capture_terminal_patch` (explicit `integrate_delegated_patch` disposition), plus the exact SKILL-PAYLOAD delegation cluster (payload snapshot, `resource_ref`, live apply under whole-payload content-hash CAS, reconcile on outcome). Full contract in §6 Delegated subagents |
      │   ├── subagent_integration.py ← integrate_subagent_patch: parent's manifest-first integration of an acting subagent's workspace.patch — self_worktree children apply into ctx.active_repo_dir() (sha256-verified, 3-way --index, protected-path gated, top-only lineage, genesis refused), stages but never commits; external_workspace children get a verified audited verdict without re-applying; NON-workspace parent integrating a COOP child gets a read-only verification + `coop_already_in_tree` no-op verdict (checkpoint-committed at root finalization). Also compare_subagent_patches: read-only best-of-N synthesis helper |
...
Completion is ONE HTTP conversation on every host: `POST /api/onboarding/complete` replaces the earlier settings+runtime-mode pair (whose inter-write failure left providers saved and runtime not). Fixed order: re-prove install-time status server-side → validate through shared setup validator + structural startup gate → read ONE live agent-account/model snapshot when subscriptions were declared → compile install preset → provider normalization FIRST then preset keys → persist settings + next-boot runtime mode + fresh-install safety default + preset marker + completion fact in ONE write (eligibility re-proved under the settings lock) → start supervisor. Install time = three proofs together (no recorded completion, no preset generation, no `settings.json`). `GET /api/onboarding` is a pure read (never persists). No second completion path; the desktop save_wizard bridge is window-lifecycle only. A completion failing AFTER the bytes land reports saved + the failing stage; only the exact success envelope is a completion. A provider save can start the previously absent supervisor; the overlay frames the wizard sandboxed WITH popup permission (the sign-in link is the primary action). Full contract: `gateway/onboarding.py`. |
...
Accounts are grouped into one card per agent family: family name, aggregate status (signed-in AND enabled; all-disabled own state), fail-safe "Next up" badge (dual-wire reader: unified `accountPools` first, legacy `next_up` second), and add action. Rows are ONE type on both engine generations: a UNIFIED engine gives every account a named row (Enabled toggle = engine's per-profile PATCH, Remove); a LEGACY engine keeps the native pseudo-row with only its ACTIONS differing (no Remove/toggle — that engine has no route). Row status semantics: `verification=not_run` neutral, `failed` error; `availability=unknown` + `not_run` → "Login status unknown" + Refresh, never a new sign-in. Second line = muted human metadata (quota, rotation exclusion, last verification time). Typed quota absence appended per subject; prose never selects semantics. Known migration-window residuals (legacy `''`-keyed quota alias for `<harness>-default` only; pinned route health applies no legacy alias) are accepted. Removal = daemon's credential-profile contract with full receipt preserved. One service banner explains daemon/runtime problems per facet; per-facet independence is real on both sides (fanned-out reads stamped into `reads` block). Full contract: `gateway/claudexor_accounts.py` + `claudexor_status_store.js`. |

Connect is link-first and harness-agnostic: typed disclosure renders sign-in URL + one-time code; optional pasted-callback field visible while active. `setupLogin` on each exact harness row is optional-without-default (`{mode: in_app}` = omitted transport, `{mode: external_terminal}` = `client_pty`; malformed = capability gap); explicit null delegates to the exact engine's typed setup/profile admission (stamped `setup_job_admission`). Explicit `client_pty` recovery stays exact even when normal mode is `in_app`; Codex pairs it with `browser_redirect`, non-Codex never acquires `loginFlow`. Typed `credential_profile_required` + `add_named_account` selects name-the-account face; typed duplicate profile idempotent directly; legacy generic 409 idempotent only after exact harness/profile read-back. Terminal-transport codes offer explicit external-terminal continuation through the same release guard; one hidden vendor-CLI install + one retry is consented only by a structural `not_supported` first-create terminal job (installer = hard-timeout new process group, stdout parsed as one strict JSON success object requiring `installedBinary` + `installedVersion`). No embedded terminal surface; terminal job state and account row are reconciled so a stale verification read can't claim failure after connect. Full contract: `gateway/claudexor_accounts.py` + `claudexor_daemon.py`. |
...
#### Engram — the external durable memory store

Engram is a separate service (its own SQLite store, HTTP on `127.0.0.1:7437`) that holds the agent's durable MEMORY: verified outcomes, learned facts, verdicts, episodic summaries. It is reached over HTTP through a standalone module family (`engram_client` / `engram_sink` / `engram_read` / `engram_cache` / `engram_conflicts` + the read-only `engram` tool) that deliberately does NOT route through `mcp_client.py` — an external memory store is a first-class dependency of the agent's cognition, not one more tool behind a transport manager. The project scope is pinned from the repository, never from cwd, and a bare drive root resolves through `OUROBOROS_REPO_DIR` for the same reason: filing one system's memories under `.../data` would split them across two projects.

**The memory/to-do boundary is the design's spine (C18).** Engram stores what was LEARNED and what was DECIDED; it never stores what to DO next. The backlog stays a local action queue, `review_evidence.open_obligations` stays local, and a reflection's `proposed_next_step` is dropped at the sink. The sink refuses to-do-shaped content outright, and the adapters select their fields from an allow-list rather than forwarding whole payloads, so a new work-item key cannot reach the store by default.

**Nothing is lost when the store is down.** Every write is spooled locally (append-only, bounded, idempotent by identity) before the network call; the next run drains it. An APPEND whose base could not be read is DEFERRED rather than guessed: the merge happens at forward time, when the base can be read, and re-forwarding a fragment the record already carries is recognised as already-applied — a blind merge would upsert over content nobody saw, and a refusal would drop the fragment for the whole outage.

The dedicated `logs/engram.jsonl` channel now carries three distinct disclosure shapes, never `logs/events.jsonl` (whose consumers reason about the ORDER of its records, so a memory-transport notice appended there would displace their last-event assertions): `engram_unavailable` (a transport failure), `engram_emit_capped` (the per-run emit cap, naming the `kind`/`identity`/`title` of the first record that was actually lost — "the cap was reached" is not something an operator can act on), and `engram_emit_filtered` (the C6 "filtered" limb: the first record refused by a gate — C18 to-do / to-do-only fields / empty / unknown kind / the S1-reversed `identity_local_only` — with its reason and refused identity). A cap is not an outage, and a gate refusal is neither, so each gets its own `type` instead of being folded into the transport notice. The identity manifest is LOCAL-ONLY by owner decision (S1 reversed): the fixed manifesto has no search scenario and no Engram consumer, so no identity-keyed record may exist at all — the sink refuses a reflection-nominated `identity_update_candidate` and any record addressed at `identity` or `identity:*` with reason `identity_local_only`, and `identity_journal.jsonl` keeps the local write's provenance with no mirror outcome to record.

**Engram assigns `review_after` only to the types in its own `decayReviewAfterMonths` map** — `decision` (6 months), `policy` (12), `preference` (3) — and types absent from that map get NULL. Every type Ouroboros writes (`dialogue_summary`, `scratchpad_block`, `identity`, `knowledge`, `review_verdict`, `evolution_outcome`, `episodic_memory`, and the LLM-nominated action types) is outside it, so `GET /review` is EMPTY BY CONSTRUCTION on this deployment: the background prompt's review section, `mark_reviewed`, and the bounded unread pushback (`REVIEW_UNREAD_STREAK_LIMIT`) are therefore all inert until the decay map is extended or the written types change. No code change is required in `due_for_review`/`mark_reviewed` for either option — both are correct as written and simply receive an always-empty due set (an empty due set is also why the unread streak resets rather than carrying over) — and the code is deliberately left in place pending the owner's decision.

**Reads are progressive and typed (C20/AC23).** Discovery returns titles under an explicit small limit, the neighbourhood layer a small radius, and a full record one at a time, with a per-turn budget (3 records, and a whole-turn character ceiling equal to the largest prompt section a retrieval may stand in for) so a turn cannot quietly re-inflate the context this subsystem exists to shrink. `ok`/`empty`/`rejected`/`unavailable`/`stale` are five different answers — the store answered and holds nothing, the store answered and REFUSED the request, the store could not be read, and the store could not be RE-read so the last good read is being served — and the windowed type-filtered reads disclose when their window saturated instead of implying they saw everything. `stale` is content served from the `engram_cache` read-through cache because the store could not be RE-read, and it carries the failed read's own cause — "the store refused this request" (a scope/session problem) and "the service could not be reached" (an outage) send the operator to different fixes, so the seam renders them as different sentences rather than one blank. `empty` is deliberately NEVER cached — `cached_read` stores only `status == "ok"` — because a blank is not content: caching one would hide a memory written moments later behind "there is nothing" for the whole TTL, and for a stable key in the disk mirror too. An empty read costs one bounded request to repeat, whereas the failed-refresh path is what holds content across an outage. An append whose base is unreadable, a knowledge topic with no local file, and a scratchpad block skipped as a to-do are all recorded as what they are.

The recall seam asks Engram for **OR matching** (`match_mode="any"`) because under the server's default (FTS5 AND) a natural-language question matches a record only when EVERY token of it is present, which in practice matched almost nothing and made the seam fall through to recency. OR matching makes a non-empty hit set normal, so a MULTI-WORD query renders TWO labelled halves inside the same 2,000-char budget — the matched titles plus the newest summaries as the continuity anchor the removed narrative used to carry — while a single-token query, an empty query, and a query with no alphanumeric token keep their previous path byte-identically (OR and AND agree on one token, and a query with nothing to match on is answered by recency alone without a request).

**The local write stops where the local reader stopped; it stays where local structure is the point.** The canonical knowledge topic + index write is retired (a switch, not a deletion: `CANONICAL_LOCAL_WRITE`), because its readers — the main-chat prompt, the deep-self-review pack, the checkpoint instrument, the consciousness wake-up — all read Engram now; the local files remain on disk as the pre-switch READ-ONLY archive, and `knowledge_read` resolves every ordinary topic in Engram alone: the archive is consulted only for the topics that have no record there at all (`patterns`, `index-full`) and for the backlog's own branch (the 43-topic backfill closed the gap), so a store miss is reported as not found — or as UNKNOWN when the store cannot answer — instead of serving text it may have superseded. Per-project facts keep their local store: their isolation lives in the PATH (`projects/<id>/knowledge`) and Engram has no equivalent scope, so stopping it would silently merge one project's facts into the repo-wide scope. The scratchpad, continuation narrative and review ledger keep their local writes for the same family of reasons — a bounded window with a recoverable eviction journal, node-associated provenance (`summary_ref`/`result_ref`), and a live commit-gate state machine are all structure the remote record does not carry — while each is ALSO mirrored into Engram so the memory survives losing the drive. Engram is thus both the retrieval target for reference material and the durable half of the state that must stay local.

**The distilled dialogue lives here now, addressed by interval rather than by prose.** The consolidator's blocks (100 messages → one block, blocks → one era) used to ride in every prompt as `## Dialogue History`, unconditionally and unbounded; that seam reads Engram now, so the local `dialogue_blocks.json` stops being an injection source and becomes the durable record behind it. A block is addressed by the interval it covers — the message offsets it spans, `gap_id` for the explicit memory-gap markers that have no interval at all, and the timestamp span only for blocks distilled before offsets were recorded — never by the summary text: the interval is what the block IS, and a retry that phrases the same span differently has to UPDATE that span's record instead of adding a second one for it. Two intervals must never share an address, so minute-resolution timestamps are not enough on their own (a burst of 100 messages inside one minute would label two different chunks alike, and the second write would overwrite the first), and an era, whose own range is date-only, carries the offset span of the blocks it covers for the same reason. The record's TITLE stays the human-readable span, because that is all the recall seam renders. A block is carried over at startup only when Engram does not already hold its interval, so a settled mirror normally costs one bounded read and no writes — normally, not always: the recency read is windowed and the `global` scope is shared, so a block pushed out of that window is re-pushed, which is an upsert rather than a duplicate or a loss, but still a write. An unreachable store defers the whole carry-over rather than pushing the backlog blind, which would also spool it again on every start.
...
`promote_chat_to_task`, `route_to_project`, and `steer_task` become successful only after their token-matched supervisor facts are durable in the existing task result, queue snapshot, annotation, or mailbox authority. With several possible tasks, the LLM chooses; code auto-delivers only the unambiguous one-target case. An unconfirmed or stale receipt fails visibly and cannot launch a second root as a fallback.

One owner TURN yields exactly ONE routing action, and the host enforces that floor rather than trusting the prompt's "never competing actions" (`tools/control.py::_refuse_second_routing_action`). The 2026-09-12 incident is why: one message steered a running task AND promoted a second root, both committed, and because the annotation sidecar keeps only the LATEST row per `client_message_id` the promote silently superseded the steer's receipt — two roots, and no durable trace of the decision that produced the first. The floor is armed only by a COMMITTED receipt (`delivered` / `scheduled`), recorded turn-locally at the ONE shared dispatch choke (`_emit_and_wait_for_routing`), so a steer whose target is gone, an unconfirmed delivery and a `needs_manual_target` refusal all leave the message free to take another decision — the documented "target not steerable, promote instead" recovery still works — and a later turn re-decides from current state. Deliberately NOT a cross-turn guard: a retry in a new turn is judged on current receipts, never on historical annotation rows.

These paths reuse the normal task lane and do not create a parallel scheduler or message history. A routing/promote decision turn receives host-built ground truth rather than relying on chat memory: the Main routing manifest carries each project's registry `working_dir` and bounded typed projections of recent task results (identity, outcome, workspace facts, artifact references — never raw result text), and a project-room turn additionally receives the thread's most recent task result in the same bounded form. That lookup reads the registry row's durable `last_task_result_id` pointer first (stamped at project-task finalization), fetching one file directly regardless of how many newer foreign results exist; an absent or stale pointer falls back to the bounded newest-first mtime scan, then to a disclosed full-store scan (the lazy self-heal for pre-pointer projects; with zero matching results nothing is written back, so it repeats per lookup until a matching result exists). Only the absent-pointer case writes the pointer back from the scan: a non-empty pointer that failed to resolve is typically a split-drive result whose canonical copy-back has not landed yet, and overwriting it would regress the pointer to an older result. On a Swarm router turn the host-owned room still chooses scope, but only on a genuine conflict: in a projectless room an explicitly passed `project_name` is inherited, and the project is created and bound before the root launches.
...
| OUROBOROS_VISION_CAPTION_TIMEOUT_SEC | 90 | Provider dead-socket ceiling for send-time captions and explicit VLM helper calls. A direct send-time caption has no child wrapper; explicit helpers give the killable child one fixed structural settlement margin and set the ToolEntry minimum two margins above the provider (global owner tool timeout may widen the outer envelope). Provider window is shortened before dispatch so all three stay inside the owner deadline/finalization reserve. |
...
| OUROBOROS_MAX_ACTIVE_SUBAGENTS_PER_ROOT | 6 | Active subagent cap per root task — readonly or acting (hard max 500 = `config.MAX_ACTIVE_SUBAGENTS_HARD_CAP`, shared with supervisor reject/reservation gates and the `wait_tasks` id cap; at ~500 children the `wait_tasks` compact projection can hit the disclosed 15K tool-result truncation — chunked waits + `get_task_result` are the follow-up; O(n²) active-tree scans accepted) |
...
| OUROBOROS_DELIVERABLES_ROOT | (empty) | Visible container for UNNAMED user deliverables; empty = ~/Ouroboros/Deliverables (sibling of projects root, outside repo/data, never GC-pruned). A BARE `user_files` filename lands here; an explicit placement (`Desktop/…` etc.) is honored under home as given. `user_files_path_block_reason` allows the container past the workspace-overlap guard only as a genuine sibling of the hard data/repo/budget drives. |
...
| OUROBOROS_PER_TASK_COST_USD | 50.0 | Hard per-task cost cap over the WHOLE task tree (own calls + subagents): wired as `UsageScope.root_limit_usd`, enforced pre-dispatch by the physical-attempt ledger (`reserve_attempt`), latched as the durable root budget fence on first refusal. Since v6.91 the in-task graceful stop (`task_pacing.resolve_cost_ceiling`) also binds — min(pct-of-global, cap − margin) against TREE-accounted spend — so best-effort wrap-up fires before the fence. The cap stops the task even with no finite global budget: the two axes are independent ceiling components. (Pre-v6.64 "soft threshold" semantics gone.) |
...
| OUROBOROS_OR_PROVIDER | "" | OpenRouter `provider` routing (v6.46.0): `resilience` (same-model failover on rate-limit/5xx, prompt-cache warm), `repro` (pin, no failover), or a raw JSON provider object. Gap-merged so it never overrides the anthropic `require_parameters` pin or the (sealed-artifact) reasoning-continuity `allow_fallbacks=false` pin; affects same-model provider routing only (never the model, so the P3 reviewer floor is untouched). |
...
| OUROBOROS_PRESENTATION | (unset) | **Env-only launcher export** (NOT a settings.json carrier). Set by `launcher.py::start_agent` to `desktop_window` (GUI) or `browser_fallback` (Linux headless probe); absent = plain web/headless serving process. Rendered as `runtime_env.presentation` (the PROCESS shell, distinct from per-message `owner_client`). Replaces retired `OUROBOROS_DESKTOP_MODE`/`is_desktop`; stripped by benchmark server runner + hermetic preflight scrub. |
| OUROBOROS_USER_FILES_ROOT | "" (home) | **Env-only operational override** (NOT a settings.json/UI carrier; deliberately absent from `SETTINGS_DEFAULTS`/`apply_settings_to_env`). Filesystem base for the `user_files` root, read directly by `tool_access._user_files_root`. Defaults to the owner's real home; a jailed/isolated runtime sets a scratch dir so a task cannot read the owner's real home, and unnamed deliverables derive under that jail. Unusable value falls back to home (fail-safe). |
| OUROBOROS_OBSERVABILITY_KEEP_RAW | unset | **Env-only operator debug override** (NOT a settings.json/UI carrier — deliberately absent so a self-change or non-owner save can NEVER enable secret logging). When set, persist the RAW LLM/tool payload as the authoritative observability blob. Default OFF: the authoritative blob is REDACTED (secrets masked, structure/route/non-secret text preserved per BIBLE P1); `full_payload_redacted` declares it honestly. |
| OUROBOROS_GENERATIVE_PROBE | 1 (on) | Enables the generative context-window probe machinery (`capability_evidence.probe(allow_generative=True)`): when provider metadata gives no window, an over-window request can empirically confirm ≥1M from a FREE pre-inference reject; `OUROBOROS_GENERATIVE_PROBE_CHARS` (default 5,000,000) sizes the padding; a 200 (possibly-paid accept) never auto-confirms — routes to owner-ack. Since the settings-time Max gate retirement no production surface passes `allow_generative=True` (scope-slot ack probes metadata-only); dormant machinery for tests/future owner probes. |
...
| OUROBOROS_REVIEWER_SLOTS | (empty) | (6.1) Structured reviewer-slot SSOT (`reviewer_slot_config.py`): JSON `{triad[], scope[], advisory}`; each row `{slot_id, route:{kind: api_chat/agent_session, target_id}, effort}` with a STABLE owner-assigned slot_id (never an array index). Empty = read legacy comma keys + phase-5 route envs as migration source. Malformed value refuses typed at save AND review time on every surface except task acceptance (owner-approved residual); env-apply logs and leaves legacy keys unprojected |
| OUROBOROS_SUBSCRIPTION_PRESET_VERSION | (empty) | One-shot INSTALL-TIME marker recording which generation of the agent-subscription preset this install received. Written only by `POST /api/onboarding/complete`; endpoint-authored and DISK-ONLY (`config.ENDPOINT_AUTHORED_SETTINGS`) — loader and env projection keep it out of `os.environ` both ways. Its ABSENCE authorizes nothing; install time is proved by all three of: no recorded completion, no preset generation, no `settings.json`. |
...
| OUROBOROS_ONBOARDING_COMPLETED_AT | (empty) | Durable "onboarding finished here" timestamp written by EVERY completion of `POST /api/onboarding/complete` (incl. no-subscription and skipped-preset completions). Endpoint-authored, disk-only, same treatment as the preset marker. Exists because `has_startup_ready_provider` false is a state an OLD install reaches when its key stops working: without a recorded completion that alone re-opened the install-time window and overwrote owner reviewer/subagent config. |
...
| OUROBOROS_TASK_REVIEW_MODE | auto | Task acceptance mode: `off`/`auto`/`required`. Only the root owns the verdict. `auto`/`required` host-review queued/headless/scheduled substantive roots and effectful direct turns; pure conversation and child authority skipped. `required` combines with `OUROBOROS_REVIEW_ENFORCEMENT`: advisory records/finalizes honestly after improvement; blocking repeats until evidence-backed clean acceptance or a real rail, subject to any explicit task-local pass cap. |
| OUROBOROS_SAFETY_MODE | full | (v6.54.3) Owner-only LLM-safety-supervisor coverage: `full` (every guarded call checked) / `light` (LLM check only for POLICY_CHECK integration tools; CONDITIONAL shell/verify fall to deterministic guards) / `off` (no LLM safety calls). Deterministic sandbox/protected-path/light-mode guards stay ON in every mode; non-full modes emit durable `safety_mode_skip` audit events. Changed ONLY via the audited `/api/owner/safety-mode` endpoint. (v6.82.0) `light` is the default only for a FRESH desktop-wizard setup with no prior choice; shipped default and fail-closed fallbacks stay `full`. |
...
| OUROBOROS_REVIEW_MAX_CYCLES | "2" | Shared paid-review-cycle cap (SSOT `ouroboros/review_cycles.py`). String: positive int or `unlimited`; malformed fails closed. Per-gate: plan review — panels/task; task acceptance — runs/task, `improvement passes = cycles − 1` (binds under EVERY policy incl. Required+Blocking; explicit `max_improvement_passes` wins; `unlimited` = no local cap, rails still bind); commit gate — paid triad+scope cycles per ROOT task (byte-identical staged diff refused FREE from first verdict-block; content-new rebuttal = one paid re-review; advisory exhaustion/identical-refusal discloses loudly, proceeds without buying review); skill review — dispatches per root task / manual-lane snapshot, one chunked wave = one cycle, identical snapshots replay free |
...
| OUROBOROS_REVIEW_MODEL_TIMEOUT_SEC | (unset) | Optional env-only logical review window: absent = API review settles on its transport bound / `agent_session` inherits the task absolute ceiling (owner deadline narrows both); present = narrows only the slot's logical wait, never passed as an HTTP read timeout. A live worker returns a typed in-flight actor; late result reconciles to the same retry identity; lost custody stays unresolved, never a second paid dispatch. |
...
| OUROBOROS_PREFLIGHT_TIMEOUT_SEC | 900 | Total wall-clock budget for the hermetic reviewed-change pytest preflight (`preflight_runner.run_hermetic_pytest`); covers node lane + both pytest passes (serial gets `total − elapsed`); on timeout performs bounded best-effort process-tree teardown (process group + inherited-env membership token / Windows Job Object, membership enumerated from live kernel state; a live/undeterminable member is an honest HARD BLOCK); containment is DETECTION not a kill guarantee, UNCONDITIONAL incl. green passes |
...
| OUROBOROS_AUTO_GRANT_REVIEWED_SKILLS | true | Owner-confirmed setting; default-on since v6.10.0 (installs without explicit choice enabled; existing explicit choices preserved). A fresh executable review grants only manifest-declared keys/host permissions for that exact content hash. Under `blocking`, blocker reviews are not executable and do not auto-grant; under `advisory`, blocker findings may auto-grant because the mode makes the review executable. Plain `/api/settings` POST drops it; desktop launcher bridge / web `/api/owner/auto-grant`. |
| OUROBOROS_TRUST_NATIVE_SEEDED_SKILLS | true | Named, hash-pinned exception to manual first review (v6.31.0, CHECKLISTS §Skills): when the LAUNCHER writes a bundled native skill payload (`.seed-origin`-marked), it stamps `review.json` clean with `review_profile="native_seed"` because those exact bytes passed the repo triad+scope gate; zero-grant skills auto-enable absent an explicit choice. Any payload edit flips stale; a verdict whose marker is gone reads pending. The flag acts ONLY at launcher seed/resync moments over repo-reviewed bytes (no owner settings gate needed). |
| ENGRAM_BASE_URL | http://127.0.0.1:7437 | Engram endpoint for the durable-memory subsystem. Read at CALL time (never at import), so a test or an operator can repoint it without a restart. Unreachable is a typed soft dependency: writes spool, reads report `unavailable` — never an empty result. |
| ENGRAM_PROJECT | (resolved) | Operator override for the Engram project scope. Mirrors Engram's own knob so both sides agree on the name. When unset the scope is the repo directory name (or `.engram/config.json`); a blank/`local` name is refused rather than letting the service fall back to its own cwd detection. |
| ENGRAM_HTTP_TOKEN | (unset) | Bearer token for the Engram routes that require `ENGRAM_HTTP_TOKEN` (delete/export/import). Reads and writes share it so they can never disagree about which service they are talking to. |
| OUROBOROS_REPO_DIR | (unset) | Repository root, read by the Engram scope resolver when a caller holds only the drive root (deep self-review, evolution checkpoints, background consciousness). Without it those writes would resolve their project from `.../data` and split one system's memory across two projects. |
| OUROBOROS_CONTEXT_MODE | max | Owner-selected context horizon: `max` or `low`. Ordinary Main calls build deterministic route-calibrated projections from one immutable core; unknown route tries Max (never silent 200K fallback); confirmed overflow may retry same model once with task-local Low without mutating the global. Since v6.80.0 this key is ALSO the single control over BIBLE P3 scope-review applicability (`max`: blocking ≥1M scope gate; `low`: whole-repo scope review declaredly not performed, typed skip row) — owner policy coupling; triad's blocking staged-diff review unaffected. Plain `/api/settings` POST drops it; owner endpoints/CLI control. |
| OUROBOROS_RUNTIME_MODE | advanced | Three-layer refactor axis (`light`/`advanced`/`pro`), orthogonal to review enforcement. `light` = compatibility/self-modification guard: blocks repo-mutation tools, mutative direct git, shallow argv writers with repo-local targets, post-execution repo dirtiness (no auto-rollback), and runtime_mode self-elevation through owner chokepoints; reviewed+enabled skills still execute. `advanced` evolves the application layer, blocks protected core/contract/release. `pro` may edit protected surfaces, but committing still requires the normal triad+scope gate. Owner-only (desktop native confirmation / web `/api/owner/runtime-mode`; neither mutates current boot baseline). |
...
| OUROBOROS_EFFORT_TASK | medium | Reasoning effort for task/chat. Full scale (config.EFFORT_SCALE, v6.57.0): none→ultra (`ultra` is the codex-family vendor tier above `max`). Provider adaptation scoped to the exact route/request shape, learned only after semantic success, disclosed in `usage.request_wire`; historical model-global ceilings remain diagnostic. Settings UI offers all tiers except `minimal` (per-call tactical tier). |
...
| OUROBOROS_TASK_IDLE_TIMEOUT_SEC | 900 | (v6.38.0) Activity-based idle window: a task is stopped only after NO real progress (`llm_usage`/progress events — not the unconditional liveness heartbeat) AND no progressing/queued subtree for this long. Base window floored to `max(idle, per_call_ceiling+120)`; an active main LLM call is protected by its exact-attempt typed in-flight row (spares only idle, no elapsed-time expiry); deadline/budget/cancellation/absolute ceiling remain independent. A child's settled terminal result stamps the PARENT's progress at task_done dispatch (delivery is the cue to integrate) |
...
| OUROBOROS_POST_TASK_EVOLUTION | false | Owner-gated, default-OFF post-task self-evolution envelope (V4; UI shows it with cadence as one Self-Improvement Trigger selector, backend stays boolean + `_CADENCE`). When enabled, after an eligible task the worker may ask the main-model slot (medium effort) whether to promote ONE improvement into the existing gated campaign; writes a durable request, supervisor applies on an idle tick through normal gates. Eligibility includes ordinary/trivial tasks; `every_n:1` = consider after every eligible task. Agent self-enable channels blocked by shell/browser/settings/data-write guards + SAFETY |
...
| OUROBOROS_EVOLUTION_PERSISTENT_OBJECTIVE | "" | Optional owner standing steer appended (as a non-overriding bias) to EVERY evolution campaign's objective, not only post-task ones; never overrides LLM-first promotion; any biased cycle still passes full triad+scope review. Empty = pure LLM choice. Owner-only like `OUROBOROS_POST_TASK_EVOLUTION` — same shell/browser-JS/`/api/settings` self-change detectors + SAFETY.md, so the agent cannot self-set it. |
...
Direct-provider review fallback (formerly "OpenAI-only review fallback"; legacy name kept for discoverability): when exactly ONE official direct provider is configured, `config.get_review_models()` compiles its declarative reviewer-role sequence with provider-prefixed model IDs (official OpenAI, Anthropic, MiniMax, Cloud.ru, and GigaChat; OpenRouter/legacy-base/compatible/mixed stay outside — `_exclusive_direct_remote_provider_env` returns empty for those). OpenAI+Anthropic run three Main slots; MiniMax mixed Main/Light; Cloud.ru/GigaChat one role model for every slot. Requires the main model to already start with the exclusive provider prefix (via `migrate_model_value`; no cross-provider free-text). Part of the single-provider independence invariant (DEVELOPMENT.md).

GigaChat provider specifics (`gigachat::`): routed through the native `gigachat` library in `llm.py::_chat_gigachat` (NOT OpenAI-compatible). OpenAI `tools` → GigaChat `functions`; at most ONE `function_call` per turn (parallel calls collapse); role `tool` → `function` (plain text wrapped as `{"result": ...}`); `system` must be first (later reminders demoted to `user`); `reasoning_effort` intentionally omitted (GigaChat-3 can burn the whole budget on hidden reasoning). Fresh direct-only installs use `GigaChat-2-Max` for every slot (GigaChat-3-Ultra limited to personal Freemium). No live cost source → cost nullable/unknown. Below the 1M scope-review floor; the v6.80+ disclosed fallback where no ≥1M reviewer is configured is owner-selected `low` mode (typed `skipped_low_context_mode` rows), with the v6.87.6/v6.89.0 retrieving-scope-slot alternative at ≥200K; the blocking triad still reviews the full staged diff in both modes. |
...
`build.sh`, `build_linux.sh`, `scripts/build_appimage.sh`, `scripts/build_linux_packages.sh`, `scripts/smoke_linux_packages.sh`, `build_windows.ps1`, and `scripts/build_repo_bundle.py` are release-invariant owners. Linux PyInstaller runs under the same pinned portable Python shipped in the payload (libpython keeps the payload's glibc floor); AppImage wraps with digest-pinned tool/runtime bytes; native packages declare external Git, keep bundled Python/Node/browser under `/opt/ouroboros`, install the opt-in user unit at `/usr/lib/systemd/user/ouroboros.service` (no activation scriptlet); release-gating smoke installs via `apt`/`dnf` and proves Git/desktop/unit/CLI/bounded launcher start (Astra/RED vendor runs are informational); macOS keeps app + Applications symlink + optional CLI installer; Release tag prerequisite: `scripts/build_repo_bundle.py` is the release-tag SSOT gate — it verifies the annotated `v$(cat VERSION)` tag points at `HEAD` before packaging. Full contract: DEVELOPMENT.md §Build & CI. |
...
The release proof begins with the final DMG, AppImage, tarball, or ZIP rather than its staging directory. Each platform shard checks the embedded repo bundle, packaged CLI, and managed Claudexor seed + Node (start owned daemon, complete fixture task, verify identity-bound stop); Linux AppImage is extracted for metadata/SBOM then run through the FUSE-independent runtime to prove exact version, CLI dispatch, browser-fallback readiness, payload lifetime, main-executable libraries, and clean shutdown (payload stays read-only at runtime; nested cleanup proof follows runtime → AppRun custodian → launcher and requires both extraction + private base absent). macOS smoke verifies Finder surfaces. Linux proven tarball is wrapped into the three native packages, each with digest-bound package-manager smoke receipt + provenance attestation. A digest-pinned Syft build produces CycloneDX inventories from extracted payload bytes (tarball inventory reused for the three wrappers); GitHub SBOM attestations bind the payload inventory to each final asset. The release job accepts only the seven expected assets, recalculates digests, verifies both predicate types, writes the checksum/evidence capsule, and rechecks the remote tag immediately before publication. Signing credentials remain step-scoped and absent from SBOM/attestation steps. |
...
| `ChatInbound.client_surface` (additive-optional) — per-message sending-surface observables, host-stamped `received_at`, rendered as runtime `owner_client` (non-web ingress gets `{channel}` fallback). | `client_surface.py`, `contracts.py`, `api_types.js` | parity + contracts + message-bus/context tests |
| `ChatOutbound.cancelable` + `TaskCancelResponse.cascade` (v6.82.0, additive-optional) — host-attested marker gating the chat Cancel action + echoed cascade flag. | `contracts.py`, `api_types.js` | parity + `tests/test_task_cancel_endpoint_v682.py` + `test_gateway_history.py` |
| `ChatOutbound.executor_route` (phase 6, additive-optional) — opaque harness id of the DISPATCH decision, NOT an execution receipt; empty/absent = native path, UI draws no chip. | `agent.py`, `gateway/history.py`, `contracts.py`, `log_events.js` | `tests/test_claudexor_owned_daemon.py` + `web/tests/review_truth.test.js` |
| `ChatOutbound.execution_evidence` (v6.89.x, additive-optional) — completion-seam RECEIPT beside the dispatch decision: `delegated_runs_*` counters + `actual_substrate` (`harness_used`/`harness_attempted`/`native_only`), `evidence_read_failed`, `nanny_nudge_recorded`, `subscription_cost_usd`, `applied_access_profiles`; unreadable custody log omits the substrate claim; counters are DELEGATED-RUN facts only. Full field semantics + layered chip rules in owner code (`delegate_evidence.task_execution_evidence`, `subagents.envelope_from_task`, `log_events.executorChip`). | `ouroboros/delegate_custody.py`, `subagents.py`, `supervisor/events.py`, `gateway/history.py`, `contracts.py`, `log_events.js` | `tests/test_execution_evidence.py`, `web/tests/review_truth.test.js`, `tests/test_task_status_flow.py` |
| `TaskDetailResponse` + root-only `TaskCostBreakdown` (read-time, never persisted) — full field set incl. `accounted_upper_bound_usd` (C2: accounted subtree UPPER BOUND, never a receipt), `subscription_sessions`, `unknown_unmetered`, `non_final_rows`, `cost_final`, `authority="physical_attempt_ledger"`; non-root/unavailable omits the object, never `$0`. Additive `cancel_state`/`cancel_reason` projection. Browser consumer: `log_events.taskCancelPending`. | `gateway/contracts.py`, `gateway/tasks.py`, `api_types.js`, `log_events.js` | parity + `web/tests/cancel_run.test.js` |
| S3 stop/hurry (v6.10x, additive) — `POST /api/tasks/{id}/hurry` (`TaskHurryRequest` exactly `{request_id}`; `duplicate` = idempotent success), `TaskCancelResponse.{cancel_state, stop_policy}`, `TaskDetailResponse.{stop_policy, owner_hurry, owner_hurry_history}`. Browser consumers: `log_events.taskSoftStopPending`, `ownerHurryProjection`. | `gateway/contracts.py`, `gateway/task_hurry.py`, `api_types.js`, `log_events.js`, `task_control_menu.js` | `tests/test_owner_hurry_s3.py` + `test_owner_stop_s3.py` + `web/tests/task_control_menu.test.js` |
| In-flight chat activity ABI (additive-optional) — `StateResponse.active_direct_turns`/`active_chat_activities` (`kind="managed_task"`, phase `queued`/`budget_paused`/`working`/`finalizing`), `TypingOutbound` + `ChatOutbound` optional `task_phase`/`task_terminal_status`. Consumers: `chat_activity.computeDerivedChatStatus`, snapshot hydration. | `contracts.py`, `api_types.js`, `supervisor/active_activity.py`, `gateway/state.py`, `chat_activity.js` | parity + `tests/test_direct_activity_registry.py` + `test_project_chat_continuity.py` + web tests |
| `project_thread` (additive-optional) — stamp on outbound frames at the bus choke when final `chat_id` is a reserved Project thread; Main's fan-out gate `chat_activity.mainThreadAccepts` never adopts it. | `message_bus.py`, `projects_registry.py`, `contracts.py`, `api_types.js`, `chat_activity.js` | `tests/test_message_bus.py` + parity + `web/tests/chat_thread_routing.test.js` |
| Structured chat delivery ABI (additive-optional) — media `task_id`/`size_bytes`/`download_url`; `LinksOutbound` (`LinkAction` exactly `{label,url}`, ≤12 actions). Producers validate/persist/replay; consumers `chat.js`/`chat_media.js`. | `contracts.py`, `api_types.js`, `message_bus.py`, `chat_delivery_events.py`, `history.py`, `chat.js`, `chat_media.js` | parity + producer/persistence/replay/browser tests |
| Owner quiz ABI (Q-2a/Q-2b) — `QuizOption`/`QuizOutbound`/`quiz_state`; ONE ingress `POST /api/decisions` (`quiz:`/`routing:` families; `interaction:` reserved); request-id idempotent, first-answer-wins; `option_index` optional for quiz (comment answer allowed), required for routing. Escalation hierarchy: root asks owner, subagent escalates to nearest LIVE ancestor. Full semantics in `ouroboros/owner_quiz.py` + `gateway/task_decision.py` + `chat_decision.js`. | `gateway/contracts.py`, `event_bus.py`, `tools/core.py`, `owner_quiz.py`, `gateway/task_decision.py`, `queue_transitions.py`, `history.py`, `chat_decision.js` | parity + contracts + `tests/test_quiz_display.py`/`test_quiz_answer.py` + `web/tests/chat_decision.test.js` |
| Managed update ABI — `UpdateMergePlan`, pinned apply request (`strategy`, base/target SHAs, recovery confirmation), typed variants, `update_status_ready` WS. | `contracts.py`, `api_types.js` | parity + `tests/test_update_apply_routing.py` |
| `ChatOutbound.review_projection` (v6.65.0, additive-optional) — compact panel/actor truth incl. bounded `actors[].findings` + exact omitted count; raw reviewer output stays in private audit storage. | `contracts.py`, `review_substrate.py` | contracts + parity + `tests/test_review_substrate_v2.py` + `web/tests/review_truth.test.js` |
| Skill review gate `preflight_failed`/`preflight_failed_stale` (additive-optional, #335) — typed browser-consumed facts; presence semantics in `skill_review_status.py`. | `skill_review_status.py`, `api_types.js` | `tests/test_contracts.py` + `test_skill_preflight_repair.py` + web tests |
