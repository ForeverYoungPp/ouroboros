# Ouroboros v6.114.17 — Architecture & Reference
...
The dedicated `logs/engram.jsonl` channel now carries three distinct disclosure shapes, never `logs/events.jsonl` (whose consumers reason about the ORDER of its records, so a memory-transport notice appended there would displace their last-event assertions): `engram_unavailable` (a transport failure), `engram_emit_capped` (the per-run emit cap, naming the `kind`/`identity`/`title` of the first record that was actually lost — "the cap was reached" is not something an operator can act on), and `engram_emit_filtered` (the C6 "filtered" limb: the first record refused by a gate — C18 to-do / to-do-only fields / empty / unknown kind — with its reason and refused identity). A cap is not an outage, and a gate refusal is neither, so each gets its own `type` instead of being folded into the transport notice. The identity manifest is mirrored to Engram too, under the namespaced topic key `identity:manifest` with scope `global` — namespaced because a reflection-nominated `memory_action` with topic `identity` would otherwise upsert straight over the manifest — and that mirror's outcome (`sent:` / `refused:` / `failed:`) is recorded in the local `identity_journal.jsonl`, because a refused or capped mirror returns before the spool append and would otherwise be indistinguishable from a delivered one.
...
**The local write stops where the local reader stopped; it stays where local structure is the point.** The canonical knowledge topic + index write is retired (a switch, not a deletion: `CANONICAL_LOCAL_WRITE`), because its readers — the main-chat prompt, the deep-self-review pack, the checkpoint instrument, the consciousness wake-up — all read Engram now; the local files remain on disk as the pre-switch READ-ONLY archive, and `knowledge_read` prefers the live store for a topic the store holds (a local-first read would serve stale text for anything overwritten since the switch). Per-project facts keep their local store: their isolation lives in the PATH (`projects/<id>/knowledge`) and Engram has no equivalent scope, so stopping it would silently merge one project's facts into the repo-wide scope. The scratchpad, continuation narrative and review ledger keep their local writes for the same family of reasons — a bounded window with a recoverable eviction journal, node-associated provenance (`summary_ref`/`result_ref`), and a live commit-gate state machine are all structure the remote record does not carry — while each is ALSO mirrored into Engram so the memory survives losing the drive. Engram is thus both the retrieval target for reference material and the durable half of the state that must stay local.
