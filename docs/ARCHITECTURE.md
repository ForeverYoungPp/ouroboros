# Ouroboros v6.110.1 — Architecture & Reference
...
server.py (Starlette+uvicorn) ← HTTP + WebSocket on configurable host:port (default localhost:8765; Docker/non-loopback supported via OUROBOROS_SERVER_HOST=0.0.0.0)
  ...
  └── ouroboros/               ← Agent core (runs inside worker processes)
      ...
      ├── nanny_pacing.py     ← Pure/process-local configured-session pacing marks: physical delegate verbs and ordinary host coordination advance the same metered-burn baseline without deciding topology or forcing a leaf.
      ...
      ├── subagent_dispatch_notes.py ← Child-facing compatibility dispatch notes and typed blocked outcome; configured session nannies receive an actor-first pending-route receipt instead of the historical "plan delegation first" instruction. `agent` keeps the historical re-exports.
...
Simple text messages may appear immediately as pending local bubbles and reconcile against their echoed `client_message_id`. Delivered documents use a durable download URL where available and rebuild from history without persisting base64; the desktop shell opens or downloads them through the host bridge so WKWebView is not navigated into a file, while a real browser uses a fetch/blob fallback. Photos and videos keep base64 in the live frame only, while supported media is stored under the canonical task artifact root with a content-addressed URL so history replay, reconnect, and older-message rebuilds restore the same bubble. The supervisor transport owns that copy so file-backed media, in-memory screenshots, ephemeral turns, and split child drives all converge on the canonical data root; unsupported or failed persistence remains an honest caption row and cannot finalize a task card. Presentation must not imply that a socket write is durable acceptance: routing and terminal truth come from host receipts, durable chat history, task records, and subsequent state reconciliation.
...
An actor-first physical start proves its actor/work order against the task-start
snapshot. For a root-direct bounded start, or for a same-nanny replacement after
...
For a configured session row, a new task first receives one ordinary host actor
episode. The actor may schedule host-visible descendants, publish evidence, call the
...
valid terminal zero-run row still wins over unrelated malformed rows. This is
...
exact run/route facts and the actor can enter the existing supervising wait.
`started_uncustodied` is a startup custody fault, not a healthy third state: the exact
run/invocation is surfaced, quiet supervision does not begin, and no replacement may
start until the original run is proven absent or terminal and any captured physical
result is explicitly disposed. Recovery replays its
stored canonical request and idempotency key rather than issuing a second semantic
start.
...
**Nanny economics are structurally quiet.** A configured actor first spends an ordinary
host episode deciding whether to coordinate, create children, or start the physical leaf;
the configured bridge then sends the complete canonical parent brief and keeps any
coordination appendix separately hashed. While the leaf is healthy, supervision performs
zero LLM calls regardless of journal activity. On a real wake the nanny uses the parent's captured model and effort, with full ordinary reasoning power,
...
| CLAUDE_CODE_MODEL | claude-sonnet-5 | Anthropic model for Claude Agent SDK advisory/review internals (values: sonnet, opus, `opus[1m]`, or full model name; the `[1m]` suffix is a Claude Code selector that requests the 1M-context extended mode) |
...
| OUROBOROS_EFFORT_TASK | medium | Reasoning effort for task/chat. Full scale (config.EFFORT_SCALE, v6.57.0): none, minimal, low, medium, high, xhigh, max. Provider adaptation is scoped to the exact route and request shape, learned only after semantic success, and disclosed in `usage.request_wire`; historical model-global ceilings/floors remain diagnostic only. The Settings UI offers all tiers except `minimal` (a per-call tactical tier, not a standing default). |
