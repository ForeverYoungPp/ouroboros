...
server.py (Starlette+uvicorn) ← HTTP + WebSocket on configurable host:port (default localhost:8765; Docker/non-loopback supported via OUROBOROS_SERVER_HOST=0.0.0.0)
  ...
  └── ouroboros/               ← Agent core (runs inside worker processes)
      ...
      │   ├── claudexor_accounts.py ← (D30) Agent accounts HTTP surface (Settings → Agents → Accounts): six thin proxies of the owned daemon's account truth — status (catalog/accounts/quota facets + `unified_accounts` feature fact), wake, login (Connect intent + setup job create/cancel/input/reconcile), credential-profile delete/patch. Zero auth logic here; full contract in §3 Settings/onboarding + `gateway/claudexor_accounts.py` |
