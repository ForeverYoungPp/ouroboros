# Ouroboros v6.111.0 — Architecture & Reference
...
server.py (Starlette+uvicorn) ← HTTP + WebSocket on configurable host:port (default localhost:8765; Docker/non-loopback supported via OUROBOROS_SERVER_HOST=0.0.0.0)
  ...
  └── ouroboros/               ← Agent core (runs inside worker processes)
      ...
      ├── review_execution_projection.py ← Pure read-side `executions[]` projection shared by Skill Review, plan review, and task acceptance; it admits only returned usage or a resolved delegated route and never invents execution, money, or profile facts.
