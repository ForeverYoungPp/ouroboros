...
| Advisory pre-review (`tools/claude_advisory_review.py`) | Two delivery classes: an `api_chat` row runs the bounded NATIVE inspection episode (governance docs reached through its read-only tools); an `agent_session` row receives a resolvable pointer marked MANDATORY FULL READ and the session reads the full doc itself — retrieval is disclosed (native reads are host-observed; vendor-session reads are not). Both classes also retrieve their EVIDENCE (`evidence_by_retrieval`): the touched-file pack — full content of every changed file, ~940K-1.15M chars with no TOTAL budget — is replaced by a `vcs_diff`/`read_file` recipe, and only the staged diff (the bounded subject) stays inlined | same two delivery classes | same two delivery classes |
