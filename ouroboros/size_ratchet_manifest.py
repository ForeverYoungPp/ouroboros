...
BAND_PATHS = {
    ...
    "ouroboros/tools/knowledge.py": "Entered the band from 742 lines: the store-side knowledge index (INDEX_LEDGER) is seeded from the provenance log and written through by the knowledge write path, and knowledge_list lists it — the tier-0 enumeration BIBLE.md:116-118 requires, kept beside the topic store whose writes it records so the seed and the index cannot drift apart.",
