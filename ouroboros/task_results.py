...
# Cost-field substrate, relocated from ``ouroboros/cost_projection.py`` so the
# retained task-result vocabulary no longer imports a module slated for
# whole-module deletion. A self-contained leaf (no ouroboros imports); every
# name keeps its exact semantics.
...
# closed whitelist each. Adding a new marker means adding it here, once.
...
def with_cost_aliases(fields: Optional[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    passes through here so both spellings leave with the same value — and it
    must be the LAST step after any cost mutation, or a later edit to one
    spelling persists a diverged pair. A name that is absent on both sides
    STAYS absent — aliasing never invents a field, and an explicit None mirrors
    as None. Precedence is :func:`resolve_cost_pair` (deprecated wins), which
...
# DERIVED from the cost SSOT (``COST_ALIAS_PAIRS``/``COST_OPENNESS_FIELDS``
# defined above) rather than re-typed: both alias spellings (C2, owner 10=B —
# the additive HONEST names for what ``cost_usd[_with_children]`` always were,
# plus the deprecated aliases that stay outbound until a separately approved
# ABI break) and EVERY accounting openness/integrity marker. Hand-maintained
# copies are how a marker reaches one surface and not the next:
# ``non_final_rows`` rides with ``cost_final`` because it is that flag's
# DISCLOSED CAUSE (v6.89.0 panel D2), and ``ledger_integrity_degraded`` was
# produced by the authority but named in no list at all, so it never reached
# any surface.
