"""Local file memory channels: identity, scratchpad, WORLD, dialogue, chat generations.

Migrated verbatim from ``ouroboros.memory``, plus the file/config utilities of
``ouroboros.consolidator`` (``_rebuild_knowledge_index``,
``_ordered_chat_generation_paths``, ``_resolve_generation_segments``,
``_consolidation_route``, ``CONSOLIDATION_REASONING_EFFORT``), so the local
file channel keeps one home after the module cleanup.

Design spec §5.15 and BLS P0 keep these files as the SSOT: Engram mirrors and
indexes identity.md, scratchpad.md, WORLD.md, dialogue_blocks.json and the chat
generations — it does not replace them.  ``Memory`` therefore stays a
file-backed implementation and every caller imports it from here.
"""
...
# --- Migrated verbatim from ouroboros.consolidator: file/config utilities ---
