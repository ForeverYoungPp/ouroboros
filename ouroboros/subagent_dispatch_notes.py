    ...
    if decision.executor == "harness":
        ...
        if configured_atomic:
            return (
                ...
                "publish evidence, start the snapshotted leaf with delegate_start, or make a "
                "typed startup/wake receipt alone is not a zero-run decision; record the typed zero-run "
                "decision through verify_and_record(contract_kind=delegation_zero_run, "
                "zero_run_decision, zero_run_basis). The typed startup/wake receipt "
