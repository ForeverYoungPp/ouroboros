...
def test_startup_settles_only_tokenless_process_local_rows(tmp_path):
...
def test_startup_terminalizes_tokenless_custody_loss_and_is_then_idempotent(tmp_path):
    ...
    assert state.reconcile_process_local_review_custody_on_startup()
...
def test_startup_terminal_tokenless_attempt_no_longer_blocks_a_new_commit(tmp_path):
    ...
    state.reconcile_process_local_review_custody_on_startup()
...
def test_startup_terminalizes_legacy_paid_attempt_with_no_roster():
    from ouroboros.review_state import AdvisoryReviewState, CommitAttemptRecord
    ...
    changed = state.reconcile_process_local_review_custody_on_startup()

    assert changed == [state.attempts[0]]
    assert state.attempts[0].status == "failed"
    assert state.attempts[0].late_result_pending is False
    assert state.attempts[0].paid is True


def test_startup_recovers_start_requested_token_by_reserved_operation(tmp_path):
