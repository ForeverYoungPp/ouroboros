...
[![Version 6.114.47](https://img.shields.io/badge/version-6.114.47-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.114.47/Ouroboros-6.114.47.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.114.47/Ouroboros-6.114.47-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.114.47/ouroboros_6.114.47_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.47/ouroboros-6.114.47-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.47/ouroboros-6.114.47-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.47/Ouroboros-6.114.47-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.47/Ouroboros-6.114.47-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 6.114.47 | 2026-09-14 | **fix(loop): the transcript cache anchor is append-only; fix(evolution): the cleanup stashes the leftovers it declines to rewind; fix(review): a retrieving advisory lane fetches the touched pack.** The prompt cache's per-round resealing rewrote an OLD message's bytes, so the wire prefix diverged at the previous anchor and every byte after it was re-read uncached: measured on evolution task 79cd720f round 6 from the persisted request blobs, 25,765 prompt tokens in that one round (~18% of the task's total uncached input) and again on every move. Anchors are now only ADDED, up to the provider's breakpoint budget — an old anchor keeps the prefix it holds — and the three rotation pins were rewritten to assert that property. The published-line guard of 6.114.43 correctly refuses to move HEAD when someone else moved the branch tip, but it declined the STASH too, so a cycle's unreviewed files stayed in a tree the follower then refused to run in (cycle 55 on the live drive, where `skipped` quietly became `stuck`); the stash now runs before the published-line question because it moves no ref, and the transaction reports `stashed_behind_upstream` with the files recoverable from the stash. The advisory prompt inlined every changed file's full body — 1,456,633 chars with an EMPTY omission list over this repo's own last commit set, which is what the live 1,357,466-char advisory transcript was made of — and now hands a RETRIEVING reviewer a 619-char recipe instead, since that reviewer carries `vcs_diff` and `read_file`; the staged diff stays inlined (it is the subject, and it is bounded) and a non-retrieving delivery keeps the historical form. |
| 6.114.46 | 2026-09-14 | **fix(llm): a missing credential names the route it actually failed on; feat(recall): the per-turn title list carries fifteen items.** The task-acceptance panel's three seats — `google/gemini-3.7-flash`, `openai/gpt-5.6-terra` and `anthropic/claude-opus-5`, every one of which `_parse_provider_model` resolves to provider `openrouter` — each reported the SDK's "Missing credentials … OPENAI_API_KEY" into the OWNER-VISIBLE DEGRADED text, naming a key their real route never reads while the install's only configured credential is `OPENAI_COMPATIBLE_API_KEY`. Client construction now re-raises the SAME exception family with the resolved provider, model, base-URL presence and the credential key that provider actually reads appended, so a failed route is diagnosable from the message alone. The per-turn recall seam returns TITLES, so the owner raised it 5 -> 15: `MAX_RECALL_ITEMS` and `MAX_KNOWLEDGE_RECALL_ITEMS` are 15 and `DIALOGUE_RECALL_BUDGET_CHARS` is 2,600, sized so 15 measured title lines (~80 chars) fit ONE half of the two-half shape; each seam now declares its own ceiling, because `MAX_DIGEST_ITEMS` mirrors another module's default and is not what bounds a title list. Measured on the way: `recent()` rendered `items[:MAX_DIGEST_ITEMS]`, so the recency leg silently capped the raised seam at 8 until its fetch bound and its render bound were separated. Gate: the recall/cache/context/engram suites passed (158 tests, including the sizing arithmetic and a 15-block render) and the llm lanes passed (163), with lint findings unchanged from HEAD. |
| 6.114.45 | 2026-09-14 | **fix(evolution): the restart refusal reads porcelain through an UNTRIMMED runner — the 6.114.44 fix did not actually hold.** The scope reviewer on 6.114.44 was right and that release's claim was false: the predicate called `run_cmd`, and `ouroboros/utils.py::run_cmd` returns `res.stdout.strip()`, so removing the local `.strip()` changed nothing — the shared helper still ate the leading space of the FIRST porcelain line, and for the most common status code the refusal named a path that does not exist. Measured on a real repo: `run_cmd` gives `'M alpha.py'` and reported `1 uncommitted path(s): lpha.py`, while `run_cmd_raw` gives `' M alpha.py\n'` and reported `alpha.py`. The pin could not catch it because it stubbed the very helper whose contract was broken — a stubbed collaborator cannot pin a collaborator's contract — so the test now builds a real repo with a real unstaged change and runs the real runner. `utils.py` gains `run_cmd_raw` (the same checked runner, untrimmed, sharing one `_run_checked` body so the locale pin and the failure contract cannot drift), and the evolution suite's stubs were re-pointed at the helper each call truly uses. Also answers 6.114.44's first advisory with evidence: `ouroboros/tools/control.py` is grandfathered in `GIANT_PATHS` and absent from `BYTE_DEBT`, so module growth needs no manifest refresh. Gate: evolution and utils suites green. |
| 6.114.44 | 2026-09-13 | **fix(evolution): a restart refusal names the condition that actually blocks it.** `_evolution_restart_block_reason` ended in one catch-all `return` whose text described whatever the author had in mind rather than the condition that fired — so cycle #41, whose reviewed commit `cef61b63` WAS `HEAD` with a matching receipt, was answered "commit_reviewed must create a local reviewed commit before evolution restart": a statement provably false at the point it was returned, which never mentioned the orphaned payload's uncommitted paths that actually blocked the boot. Each clause is now derived from the condition it tests, and ALL failing conditions are reported, because the authority check ran only on the clean branch — a task holding both a dead receipt and a dirty tree was handed one cause and would repair the wrong one. The dirty clause names the blocking paths (bounded at 12 with an exact omitted count, so a listed subset never reads as the complete set, BIBLE P1) and states the consequence honestly: an agent-requested restart boots through `safe_restart(unsynced_policy="rescue_and_block")`, so dirt makes the boot rescue-and-BLOCK and pause the campaign instead of starting the reviewed body. The sibling owner-visible message had collapsed three facts into "the live checkout no longer matches the exact reviewed evolution commit", which is false for the dirty case — the checkout DOES match there — and each fact now names itself. Parsing porcelain surfaced a latent trap — porcelain v1 puts the path at index 3, so the FIRST line of ` M path` lost a character and the refusal could name a file that does not exist — but this release removed only the LOCAL `.strip()`: the shared `utils.run_cmd` trims its own stdout, so the trap stayed live and its pin stubbed the very helper whose contract was broken. Closed for real in 6.114.45, with the measurement. Gate: 356 tests across the restart/evolution surface passed (7 new pins: the cycle-#41 case, the omitted-count disclosure, rename sources, both SHAs when HEAD moved, every-failing-condition, the supervisor-side dirty-tree refusal, and the first-path pin that 6.114.45 re-pointed at the real runner); carriers synced to 6.114.44 with `version_carrier_desyncs()` clean. |
| 6.114.43 | 2026-09-13 | **fix(review): a paid review episode delivers the verdict it was paid for; fix(evolution): a cycle's cleanup never moves the branch behind its published line.** Every reviewer row on a roster-backed panel resolves through the native tool-round episode, and that episode had no deliverable rail: it inspected until it ran out of rounds and then raised `native_rounds_exhausted`, so triad, scope and advisory all produced no verdict for an inspection already paid for. The LAST permitted round now withdraws the inspection tools and states why, and that deliverable turn is floored at the review surface's own output budget, because hidden reasoning shares the output budget with the answer. The rail guarantees the ATTEMPT, never a verdict: an empty deliverable keeps the same typed refusal and the round/transcript caps still fail closed. Two review-lane test fixes ride along: the operator's saved review panel is scrubbed from the test environment together with the roster it names (18 failures on a clean tree, now 0), and `test_ephemeral_lock_hardening.py` joins the serial lane. The worktree cleanup that restores an unabsorbed cycle to its recorded `base_head` guarded concurrent tasks but had no notion of the branch's PUBLISHED line, so `git reset --hard base_head` also fired when the tip had been moved by someone else. Measured consequence, twice in one afternoon: a landing made *during* a cycle (2026-09-13T22:50:17Z, on the owner's explicit manual-landing instruction) was preserved on `evolution-leftover-20260913_225131-a70edb` and then rewound off `ouroboros` 74 seconds later while `origin/ouroboros` still held a divergent tip — and every following cycle then planned and committed against a stale base (the `baseline_stale` attribution blocks and worker SHA mismatches of that session). The cleanup now asks `_rewind_is_safe_against_published_line()` BEFORE it touches anything: when the branch's upstream is not an ancestor of `base_head` the transaction does not own the workspace, so nothing is stashed and nothing is reset — the transaction records `skipped_behind_upstream`, the head it declined to move and the upstream it would have fallen behind. An unreviewed leftover above an up-to-date base, and a checkout with no upstream, keep the historical behaviour. Gate: the evolution suites passed (113 tests), including two new pins — the published-line refusal (tip, stash list and leftover refs all unchanged) and the no-over-fire case; the review fix was verified end-to-end on the real configured route (the deliverable round withdrew the tools and returned a verdict naming a value obtainable only by reading the file it had inspected) with 317 review-lane tests green. |
| 6.114.42 | 2026-09-13 | **fix(evolution): a cycle's own outcome record stops claiming the cycle CHOSE to do nothing.** The mint wrote a bare `no_op` whenever the agent's `commit_sha` was empty — evidence about AUTHORSHIP used as a claim about INTENT — and that word is fed back into the next cycle's own objective as its record of itself. Measured on the live ledger: all twelve most recent cycles read `no_op` while those same durable rows carried `execution=infra_failed` (four of them, three inside 24 seconds), `execution=cancelled` (the owner's own `/evolve stop`) and `objective=fail` with a `blocked_with_evidence` tier (measured work the commit gate refused). The outcome is now derived from the axes the transaction already holds — `interrupted` for a cancelled/interrupted execution, `infra_failed` for an infrastructure failure, `uncommitted` otherwise, which names exactly what the record proves and no more — and the projection re-derives a legacy `no_op` rather than echoing it, so the rendered digest tells the truth immediately without rewriting stored history. The vocabulary is closed (`CYCLE_OUTCOME_VOCABULARY`) with the subset consumers read (`SPENT_CYCLE_OUTCOMES`): `post_task_evolution` carried a three-name literal that silently SKIPPED every other value and would have dropped an objective out of the anti-repeat guard entirely, so widening it is now a deliberate act. `owner_stopped` is deliberately NOT spent — the owner's stop is a verdict about the campaign, not about the objective — and `waiting_for_restart` is not spent either, because an unverified commit leaves the transaction open. Live before/after on the real ledger: `Cycle outcomes (12 recent cycles): no_op=12.` became `infra_failed=4, interrupted=1, uncommitted=7.` Gate: the evolution suites passed (216 tests), including the pin that used to FEED `execution=infra_failed` and assert `no_op`. |

Older releases are preserved in Git tags and GitHub releases. Older 6.x rows (including 6.114.41, 6.114.38, 6.114.37, 6.114.29, 6.110.1, 6.108.1, 6.106.0, 6.101.1, 6.97.2, 6.105.0, 6.97.1, 6.97.0, 6.96.1, 6.96.0, 6.95.0, 6.94.0, 6.93.0, 6.92.1, 6.92.0, 6.91.1, 6.90.3, 6.91.0, 6.90.2, 6.90.0, 6.87.5, 6.87.4, 6.87.3, 6.87.2, 6.84.0, 6.87.1, 6.83.0, 6.86.1, 6.81.1, 6.76.0, 6.75.0, 6.74.5, 6.74.4, 6.74.1, 6.74.0, 6.73.2, 6.73.1, 6.73.0, 6.72.0, 6.71.2, 6.71.1, 6.71.0, 6.70.0, 6.69.0, 6.68.0, 6.67.0, 6.66.0, 6.65.4, 6.65.3, 6.65.2, 6.65.1, 6.65.0, 6.64.3, 6.64.2, 6.64.1, 6.64.0, 6.63.0, 6.62.0, 6.61.4, 6.61.3, 6.61.1, 6.61.0, 6.60.0, 6.59.0, 6.58.0, 6.57.0, 6.56.0, 6.55.0, 6.54.4, 6.54.2, 6.54.1, 6.54.0, 6.53.4, 6.53.0, 6.51.0), the 5.2.0 through 5.33.0-rc.6 rows, and former `4.0.0` rows are rolled off to respect the P9 changelog cap; their full bodies remain at their git tags.
