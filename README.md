...
[![Version 6.114.31](https://img.shields.io/badge/version-6.114.31-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.114.31/Ouroboros-6.114.31.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.114.31/Ouroboros-6.114.31-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.114.31/ouroboros_6.114.31_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.31/ouroboros-6.114.31-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.31/ouroboros-6.114.31-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.31/Ouroboros-6.114.31-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.31/Ouroboros-6.114.31-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 6.114.31 | 2026-09-12 | **fix(memory): the knowledge index claimed more about ABSENCE than a write-through index can know.** v6.114.30's listing header ended "Durable memory lives in Engram, so a name absent here was never written to the store" — literally false, because the index only sees writes that pass through the knowledge tools while the governance-doc mirror writes to Engram directly: **312 `knowledge:govdoc:*` records exist in the store and appear nowhere in this index.** A mechanism that reads one path may not speak for the whole store, so the sentence now states its own scope — "N topic(s) written through the knowledge tools", "This is a WRITE-THROUGH index, so a topic absent here was not written through this path — records another path put in the store (the governance-doc mirror, for one) are there but not listed" — and a test pins the corrected wording plus the absence of the old over-claim. The resident projection was audited for the same class and needed no change: it claims LESS than it knows ("It names what exists — it is not proof a record is still present"), never speaks about what is missing. Also recorded here, because the gate asked: a per-file size-ratchet audit of this change found **no unregistered growth of ours** — `ouroboros/tools/knowledge.py` was the only file that crossed a boundary (742 → 1,036 lines, the 1,001-1,500 band) and it was registered in v6.114.30 (`size_ratchet_manifest.py` BAND_PATHS), while every remaining offender in the marker-excluded `size_ratchet` lane is pre-existing staleness in files this work never touched: `consciousness.py` (1,804 lines, unchanged), `context.py` (1,985 lines at the parent — ALREADY a giant above the 1,600 threshold, so this change crossed no boundary there), `review_evidence.py` (1,607), `context_compaction.py` (1,014), `memory.py` (1,139), `test_engram_instruments.py` (1,485, the test file — not the module), `test_engram_sink.py` (1,166) and `test_process_custody.py` (1,025) — all byte-identical in line count across the change and all absent from the manifest before it. |
| 6.114.30 | 2026-09-12 | **feat(memory): the durable knowledge index is back in the main chat, and `knowledge_list` can finally name what the store holds.** BIBLE.md:116-118 permits a context mode only while the tier-0 core — which NAMES the "durable knowledge index" — "stays always-loaded in full"; the main chat was the one assembly that dropped it (`include_derived_knowledge=False`, which also carried the retired LOCAL archive index), while the BG lane kept rendering it through the un-gated builder. Nothing could replace it: `index-full.md` froze at the switch (its writer is retired, so it cannot name a post-switch topic), and Engram has no type-enumerating route — `/observations/recent` silently ignores `type=`/`sort=`, `/stats` carries no breakdown, `/search` needs a non-empty query. Measured on the live drive: **15 topics existed in the store with no surface that could list them.** The fix is write-through, not a read-time sweep (a `/recent?limit=500` filter is the unbounded body pull the read layer forbids, and it silently misses past its window): the write path already holds the topic, the identity and the sink's verdict, so it maintains `knowledge_index.jsonl` beside the provenance log, seeded ONCE from that log (135 rows / 60 distinct topics on this drive, including the backlog's topic-only audit rows) so a drive that predates the feature still names everything. Rows carry the transport status, so "queued in Engram" is distinguishable from "sent"; superseded rows collapse to newest-per-topic and a 400-topic cap evicts with an audited row instead of a silent drop. Reads are pure — `knowledge_list` stays a POLICY_SKIP read that creates nothing. The main chat gains a bounded projection (`KNOWLEDGE_INDEX_BUDGET_CHARS = 4_000`, sized BELOW the 6,569-char index this seam replaced, so the tier-0 element returns as a net reduction): one line per topic with its identity, plus a named omission and the path to the full index. It is injected in the main-chat dynamic list ONLY — the BG lane already spends 21,814 chars on the archive index and the Pattern Register, so a titles-only list there would duplicate most of its rows for less information each (pinned by a test that fails if someone adds it to BG). `knowledge_list` now names the STORE topics first, over the archive half filtered to the topics the store does not cover — so the three local-only topics still appear, no name is listed twice, and a topic living only in Engram is finally visible. Known bound, stated not hidden: BG keeps the pre-switch archive, so its 13-15 post-switch topics stay unnamed there until the source of the section BG already renders is repointed — a separate, smaller item. `ouroboros/tools/knowledge.py` enters the 1,001-1,500 band (742 → 1,034 lines) with its rationale recorded in the size-ratchet manifest. |
| 6.114.29 | 2026-09-12 | **fix(bg): the background consciousness injects the same bounded drive-state projection the main chat does, instead of the whole file.** BG read `state/state.json` as one string and appended it raw ("Full drive state: no clip_text here"), with `BG_STATE_JSON_WARN_CHARS` only warning: on the live drive that is a six-figure character count across dozens of top-level keys, and it was one of the two sections that overflowed the background context. The lane now calls the existing `context._drive_state_section(env)` — the same fixed key allowlist, JSON projection and P1 disclosure the main chat has always rendered, where omitted keys are NAMED and the full file stays one `read_file(root='runtime_data', path='state/state.json')` away. No new constants and no new projection code; the warn stays but now flags the FILE (which still costs a full read+parse per cycle) rather than the injected size. Scope: the shared builder, so both lanes share ONE behaviour — nothing else consumed the raw in-prompt state. Gate: the touched BG and context suites passed with no failures (junitxml); the full suite was not run, per the owner's standing directive. |
| 6.114.28 | 2026-09-12 | **feat(memory): the scratchpad mirror carries the block's own title, recall gains a knowledge leg, and the planning seam says to retrieve first (owner-approved trio; C excluded).** **A** — `_mirror_block_to_engram` titled every block the constant "Scratchpad block (<source>)", so mirrored blocks were indistinguishable in a retrieval list; the title is now the block's own first non-empty line (falling back to the constant only when there is none, since an empty title is refused by the sink), and the IDENTITY is untouched — still the content fingerprint over {ts, source, content}, so re-titling is an upsert and never forks a key. The already-stored blocks are fixed by a ONE-SHOT, SELF-TERMINATING pass (`Memory.retitle_scratchpad_mirrors`, driven only by `.ouroboros/engram-packet/retitle_scratchpad_blocks.py`, dry-run by default): it records the fingerprints it re-titled in `state/scratchpad_retitle.json`, so a second run emits NOTHING, and it is deliberately NOT a boot action — a per-boot whole-set re-emit is the shape v6.114.22 removed for dialogue blocks. **B** — the recall seam gains a THIRD leg: knowledge on the same query, titles only, its own budget (`KNOWLEDGE_RECALL_BUDGET_CHARS`) and item cap, rendered under `### Related knowledge (titles only)` and absent when it has no titles; it is additive to the two dialogue halves rather than a split of them, so the section can now cost up to the dialogue budget plus the knowledge one (docstring discloses it, and the cache cap was raised to match). **D** — prompt-only: the Task Contract Discipline section now opens with "Before planning, check `knowledge_read` and the `engram` tool …", with no gates or code paths. Gate: the touched context/engram/scratchpad suites passed with no failures (junitxml); the full suite was not run, per the owner's standing directive. |
| 6.114.27 | 2026-09-12 | **fix(tools): the ephemeral decision turn may use the `engram` tool it is explicitly told to use (owner ruling on CW3).** CW3's allowlist was a default-deny subtraction applied to the whole built-in surface, and it removed `engram` — while the SAME turn's prompt carries the Engram recall injection whose seam text reads "Relevant recalled memory - titles only. Use the `engram` tool …" (`context.py:1199`). Verified before changing anything: every `_ephemeral_turn` branch is a WRITE gate or an additive marker — `supervisor/workers.py:1300-1301` ("skips long-term memory / reflection / evolution writes"), `agent_task_pipeline.py:650-665` (no durable task record), `agent.py:680-685` (no task_result), `loop.py:669-670` (no review control), `loop_tool_execution.py:145-148` (a presentation marker), `context.py:72-80` and `:797-798` (added routing notice/rule) — and the recall section is built with no ephemeral gate (`context.py:1226-1236`), so the pointer was unactionable. `engram` is now in `_EPHEMERAL_ALLOWED_TOOLS` (read-only, self-budgeted at 3 reads / 30,345 chars per turn); nothing else was added. The v6.114.24 disclosure row follows automatically: `engram` leaves the withheld set while `knowledge_read`/`knowledge_write`/`knowledge_list`/`send_user_message`/`update_scratchpad` stay in it, and both subagent lanes are unchanged. Gate: the touched capability/policy suites passed with no failures (junitxml); the full suite was not run for this change. |
