...
[![Version 6.114.21](https://img.shields.io/badge/version-6.114.21-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.114.21/Ouroboros-6.114.21.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.114.21/Ouroboros-6.114.21-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.114.21/ouroboros_6.114.21_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.21/ouroboros-6.114.21-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.21/ouroboros-6.114.21-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.21/Ouroboros-6.114.21-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.21/Ouroboros-6.114.21-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 6.114.21 | 2026-09-12 | **hardening: the `.ouroboros/` packet is git-ignored, and knowledge topics are Engram-only on the canonical path (owner ruling, option i).** Two changes in one commit. (1) `.gitignore` gains a commented `.ouroboros/` entry, so the Engram packet with its diffs, plans and runbooks can never be swept into a commit by a future `git add -A`; nothing else in that file changes. (2) `knowledge_read`'s global path no longer consults the pre-switch local archive for ordinary topics: after the Engram read, the local file answers only for the topics whose only home it is — `patterns` and `index-full` (the latter reserved to internal readers, refused by name) — while the improvement backlog keeps its own earlier branch (C10.1). A store miss therefore reports "not found", or UNKNOWN when the store cannot answer, instead of text the store may have superseded; the project-scoped local-first path is untouched. `prompts/SYSTEM.md` and `docs/ARCHITECTURE.md` now state the rule, and the tests that pinned the old fallback were re-pinned, never weakened: the archive-fallback read became Engram-only, the exception read stays local, and the C2/P1 archive-integrity case now asserts the file byte-for-byte rather than trusting a read to prove it is still there. Gate: the touched knowledge, engram and prompt suites passed with no failures (junitxml); the full suite was not run for this change. |
