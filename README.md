...
[![Version 6.114.24](https://img.shields.io/badge/version-6.114.24-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.114.24/Ouroboros-6.114.24.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.114.24/Ouroboros-6.114.24-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.114.24/ouroboros_6.114.24_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.24/ouroboros-6.114.24-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.24/ouroboros-6.114.24-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.24/Ouroboros-6.114.24-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.24/Ouroboros-6.114.24-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 6.114.24 | 2026-09-12 | **fix(memory): the continuation note names a tool the reader actually holds, and an ephemeral turn's withheld built-ins are disclosed.** Two defects. (1) The truncation note hard-coded "continue with the `engram` tool: op='read' offset=N", but it is emitted from `knowledge_topic` — i.e. inside `knowledge_read` output — which had no `offset` argument, so a >4,000-char topic was uncontinuable for a lane holding `knowledge_read` and not `engram` (an acting subagent), and for an ephemeral turn that holds neither. `knowledge_read`/`knowledge_topic` now take the same `offset` window the `engram` tool got (head and tail markers, end disclosure, every bound unchanged), the note is surface-aware (`knowledge_read('topic', offset=N)` where `knowledge_read` produced the text, `op='read' offset=N` in the read tool), and both paths share one `_window_body` so they cannot drift. (2) `registry.schemas()` disclosed `disabled_by_contract` and `missing_credential`, and the extensions and mcp surfaces each carry an `ephemeral_turn` row — but the built-ins the CW3 allowlist withholds (the whole Engram/knowledge surface, `send_user_message`, `update_scratchpad`) produced no row at all, so a lane could only learn its own shape by probing for tools it does not hold. It now emits `{"surface": "tools", "reason": "ephemeral_turn", "tools": [...]}`, computed from the same admissibility predicate the schema filter uses, and rendered by the one `format_capability_omissions` formatter. Gate: the touched capability, engram and knowledge suites passed with no failures (junitxml); the full suite was not run for this change. |
| 6.114.23 | 2026-09-12 | **fix(memory): a truncated read now names where the record continues, and `engram op='read'` takes an `offset`.** The topic and digest text paths and the read tool cut long bodies client-side and ended them with a bare `…[truncated]` — which says the text stopped but not where to resume, even though the FULL body was already in hand (the store returns whole records; the topic/digest paths hold the search hit's record and `op='read'` fetches one outright). Every cut now carries a continuation pointer naming the exact resume offset and the surface that takes it, and `engram op='read'` gained an optional `offset` argument that returns the next window of the record with a head marker naming the continuation point and a tail marker naming the next offset; an offset past the end is disclosed as such rather than returning an empty body. Every existing bound is unchanged — the topic path's hard `<= MAX_TOPIC_CHARS` still holds because the cut now accounts for the longer note (`_truncate_body`), and the read tool's own slack still covers its markers. Gate: the touched engram, context and learning suites passed with no failures (junitxml); the full suite was not run for this change. |
