...
[![Version 6.114.18](https://img.shields.io/badge/version-6.114.18-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.114.18/Ouroboros-6.114.18.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.114.18/Ouroboros-6.114.18-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.114.18/ouroboros_6.114.18_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.18/ouroboros-6.114.18-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.18/ouroboros-6.114.18-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.18/Ouroboros-6.114.18-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.18/Ouroboros-6.114.18-linux-x86_64.tar.gz
...
| 6.114.15 | 2026-09-12 | **fix(memory): no non-successful read may be presented as `ok` or `empty` (plus one stale docstring).** Invariant sweep across `engram_read.py`: a read that did not succeed must never be reported as success or as absence. Four sites are fixed — `knowledge_topic`'s fallback scan (`if scan.ok:` only, so a FAILED scan fell through to the bounded-absence branch asserting "an exact-key scan of the newest 50 records" for a scan that never completed; it now returns `_read_failure(scan)`, isomorphic with the two sibling reads), a MATCH whose id is unusable (was `status="empty"` = claiming the store holds nothing; now `unavailable` with "the record exists and could not be read — this is not an absence"), a match whose payload never arrived (`got.ok` with no record was returned as `status="ok"` with an empty body; now `unavailable`), and the same two faces inside `continuation_narrative`. Every other absence claim in the module was audited and is reached only after a successful read (or is local input validation with `ok=False`). Known, reported, deliberately NOT fixed here: `continuation_narrative` still resolves through a `limit=8` search window with no exact-key fallback — the same structural exposure `knowledge_topic` had, with bounded wording already. Also corrected: `context_layout.py`'s TIER-0 docstring still listed the knowledge index as always-full while `TIER0_ALWAYS_FULL` excludes it and `TIER0_RETRIEVAL_BACKED` declares the demotion. Tests: the four new branches each fail against the pre-fix code (RED demonstrated on a worktree at v6.114.14) and pass now. Carriers synced to 6.114.15. |
| 6.114.14 | 2026-09-12 | **fix(memory): correct the fallback scan's bound — it was the near-ceiling pull the module's own doctrine forbids.** 6.114.13 added `MAX_TOPIC_KEY_SCAN = 200` for the exact-key miss path; that is 4× `MAX_WINDOW` and only 2.5× under `MAX_COUNT_LIMIT = 500`, and `GET /observations/recent` has no type filter and no compact mode, so every row carries its full body — i.e. "fetch the entire store's bodies in one request", the unbounded pull the three-layer design exists to prevent (`MAX_WINDOW`'s own block says so). Worse, it fired on EVERY miss, including topics never stored at all, making a large body pull the routine cost of a negative answer. The constant is GONE: the fallback now uses `MAX_WINDOW`, the module's already-justified record-shaped recency bound, at the call site, so a negative answer costs a bounded read rather than a store dump; the absence `detail` interpolates both bounds (`MAX_DIGEST_ITEMS` and `MAX_WINDOW`) so the wording cannot drift from the code. Tests assert `/observations/recent` is called with `limit=MAX_WINDOW` and the same resolved project, and that an absent topic's detail names both bounds — both still fail against the pre-fix code. Carriers synced to 6.114.14. |
