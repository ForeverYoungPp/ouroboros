...
[![Version 6.114.35](https://img.shields.io/badge/version-6.114.35-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.114.35/Ouroboros-6.114.35.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.114.35/Ouroboros-6.114.35-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.114.35/ouroboros_6.114.35_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.35/ouroboros-6.114.35-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.35/ouroboros-6.114.35-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.35/Ouroboros-6.114.35-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.35/Ouroboros-6.114.35-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 6.114.35 | 2026-09-13 | **fix(memory): the timeline route's real payload decodes, and an unreadable body is UNPARSED rather than an absence.** The `engram` tool's `op='timeline'` decoded the store's answer with a key-scan heuristic while the route actually answers `{focus, before, after, session_info, total_in_range}` — so a live neighbourhood read as a dead layer, and the tool printed an empty answer as if the store held nothing. `EngramResult.items_decodable()` and `timeline_neighbourhood()` now name the route's own shape (`TIMELINE_PAYLOAD_KEYS`), the renderer distinguishes a genuinely empty neighbourhood from one it could not parse (`ENGRAM TIMELINE UNPARSED` / `ENGRAM SEARCH UNPARSED`), and three route-specific pins hold it: an unparsed timeline body is not an absent neighbourhood, a genuinely empty one stays a distinct answer, and an unparsed search body is not "no candidates". The generalizable lesson this fixes: when a decoder is a heuristic key-scan, each route's REAL success shape needs its own pin — the old test pinned the stub's bare-list shape, which is exactly how the layer stayed dead while its test stayed green. Gate: the touched engram suites passed with no failures (junitxml); the full suite was not run, per the owner's standing directive. |
