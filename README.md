...
[![Version 6.114.4](https://img.shields.io/badge/version-6.114.4-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.114.4/Ouroboros-6.114.4.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.114.4/Ouroboros-6.114.4-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.114.4/ouroboros_6.114.4_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.4/ouroboros-6.114.4-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.4/ouroboros-6.114.4-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.4/Ouroboros-6.114.4-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.4/Ouroboros-6.114.4-linux-x86_64.tar.gz
...
| 6.114.1 | 2026-09-06 | **fix(tests): zombie-aware liveness probes for pid-1 containers.** Three test-side latent defects, all exposed where the ouroboros server IS pid 1 and never reaps orphaned children: the lifeline test's parent exited before the child armed its lifeline (parent now sleeps 3s so the child arms through the untouched `_watch` loop); the `os.kill(pid, 0)` oracle cannot see a zombie, so a completed kill read as a survival (assertions now use a `/proc/<pid>/stat` zombie-aware oracle — `tests/test_process_custody.py` lifeline + quiesce probes narrowed to that test's scope, and the same oracle in `tests/test_netres_git_bounded.py`'s bounded-git kill test). Production semantics untouched, no assertion relaxed. uv.lock version row synced; other carriers synced to 6.114.1. |
