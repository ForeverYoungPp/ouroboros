...
[![Version 6.114.28](https://img.shields.io/badge/version-6.114.28-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.114.28/Ouroboros-6.114.28.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.114.28/Ouroboros-6.114.28-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.114.28/ouroboros_6.114.28_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.28/ouroboros-6.114.28-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.28/ouroboros-6.114.28-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.28/Ouroboros-6.114.28-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.28/Ouroboros-6.114.28-linux-x86_64.tar.gz
...
| 6.114.25 | 2026-09-12 | **fix(memory): a truncated LIST body no longer claims a resume offset that addresses nothing.** `_bounded_body` renders a LIST of records (the type digests, review items, topic indexes), yet v6.114.23's note told the reader to continue "with the engram tool: op='read' offset=N" — an offset into the joined text, which addresses no record at all, so a digest cut mid-list pointed at a character index no tool could resume from. List bodies now carry a list-specific surface ("a narrower query, or the engram tool's op='read' for a single record"), while the single-record surfaces keep their offsets: `knowledge_read('topic', offset=N)` where `knowledge_read` produced the text, and `op='read' offset=N` inside the engram tool. One `_truncate_body`/`_window_body` pair still owns both the cut and the note, so the three surfaces cannot drift, and every bound is unchanged. Gate: the touched capability, engram and knowledge suites passed with no failures (junitxml); the full suite was not run for this change. |
