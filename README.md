...
[![Version 6.114.34](https://img.shields.io/badge/version-6.114.34-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.114.34/Ouroboros-6.114.34.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.114.34/Ouroboros-6.114.34-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.114.34/ouroboros_6.114.34_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.34/ouroboros-6.114.34-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.34/ouroboros-6.114.34-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.34/Ouroboros-6.114.34-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.34/Ouroboros-6.114.34-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 6.114.34 | 2026-09-13 | **fix(routing): one owner turn cannot emit two competing routing actions.** The 2026-09-12 incident — one owner message produced BOTH a `steer_task` mailbox delivery and a `promote_chat_to_task` second root, and because the annotation sidecar keeps only the LATEST row per `client_message_id` the promote silently superseded the steer's receipt — had no host fence at all: the only "never competing actions" statement lived in the prompt. The host already HELD the turn-local fact (`ctx._typed_routing_action_emitted`, set when a routing event is emitted and read at finalization) but never consulted it before a second emit, and it overwrote on each emit, so the second decision became the visible one. The floor is now enforced at the ONE shared dispatch choke: a committed receipt (`delivered` / `scheduled`) arms `ctx._committed_routing_action`, and `_refuse_second_routing_action` refuses the next routing verb before any side effect, so no second event reaches the task lane. Narrow by design — a steer whose target is gone, an unconfirmed delivery and a `rejected` receipt all leave the message free to take another decision (the documented "target not steerable, promote instead" recovery survives), and the fact is turn-local rather than keyed on `client_message_id`, which the API/CLI/scheduled lanes do not carry. Verified: nine unit cases plus the incident replayed inside `tests/test_promote_chat_flow.py`, the six affected suites green, and a before/after full-lane failure-set comparison. Disclosed residual: a duplicate arriving in a NEW turn is not fenced. |
| 6.114.33 | 2026-09-12 | **fix(health): the ledger growth note drops the last unsupported number.** v6.114.32 corrected the note's false claims, but the rendered text still carried "~1.2 KB per reservation row" — a size figure with no measurement basis, while the same line already prints the live file size computed at render time. A number without its basis is the same class, one notch finer, so the figure is gone: the note now states the mechanism (append-only and unbounded) and the live size does the measuring. Every other clause is unchanged — the per-process read cache making the steady state incremental, the COLD path with its three named triggers (restart, cache-slot eviction, read failure) under the 45s monetary lock, the absent compaction/rotation/trimming primitives, and the candidate remediations marked neither implemented. The pinning tests now also ban "per reservation row" and "KB per", so the figure cannot come back. Gate: the touched context/health suite passed with no failures (junitxml); the full suite was not run, per the owner's standing directive. |
| 6.114.32 | 2026-09-12 | **fix(health): the usage-ledger growth warning states its real cost and promises nothing that does not exist.** The rendered note asserted three things the code contradicts: that "every reservation re-reads the ledger under the monetary lock" (pre-#129 behaviour — the per-process warm cache in `usage_ledger._LedgerRowsMemo` makes the steady-state in-lock read incremental), that "ledger compaction is the remediation" (no compaction, rotation, trimming, pruning, rebuild or archive primitive exists anywhere in the usage surfaces — verified by symbol sweep), and that it was "tracked as a GitHub issue" (a dead pointer on this fork). It now names the FILE (append-only, unbounded, ~1.2 KB per reservation row), the COLD path that still costs a whole-file parse and validate under the 45s monetary lock plus a per-process ordered record table — triggered by a restart, cache-slot eviction (`_LEDGER_READ_CACHE_MAX_ROOTS` = 8) or any read failure, on an install that restarts often — and the candidate remediations as unimplemented, with no dead link. The same sentence in the sibling no-rotation note was corrected in the same pass. Gate: the touched health and context suites passed with no failures (junitxml); the full suite was not run, per the owner's standing directive. |
...
Older releases are preserved in Git tags and GitHub releases. Older 6.x rows (including 6.114.29, 6.110.1, 6.108.1, 6.106.0, 6.101.1, 6.97.2, 6.105.0, 6.97.1, 6.97.0, 6.96.1, 6.96.0, 6.95.0, 6.94.0, 6.93.0, 6.92.1, 6.92.0, 6.91.1, 6.90.3, 6.91.0, 6.90.2, 6.90.0, 6.87.5, 6.87.4, 6.87.3, 6.87.2, 6.84.0, 6.87.1, 6.83.0, 6.86.1, 6.81.1, 6.76.0, 6.75.0, 6.74.5, 6.74.4, 6.74.1, 6.74.0, 6.73.2, 6.73.1, 6.73.0, 6.72.0, 6.71.2, 6.71.1, 6.71.0, 6.70.0, 6.69.0, 6.68.0, 6.67.0, 6.66.0, 6.65.4, 6.65.3, 6.65.2, 6.65.1, 6.65.0, 6.64.3, 6.64.2, 6.64.1, 6.64.0, 6.63.0, 6.62.0, 6.61.4, 6.61.3, 6.61.1, 6.61.0, 6.60.0, 6.59.0, 6.58.0, 6.57.0, 6.56.0, 6.55.0, 6.54.4, 6.54.2, 6.54.1, 6.54.0, 6.53.4, 6.53.0, 6.51.0), the 5.2.0 through 5.33.0-rc.6 rows, and former `4.0.0` rows are rolled off to respect the P9 changelog cap; their full bodies remain at their git tags.
