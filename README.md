...
[![Version 6.114.30](https://img.shields.io/badge/version-6.114.30-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.114.30/Ouroboros-6.114.30.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.114.30/Ouroboros-6.114.30-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.114.30/ouroboros_6.114.30_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.30/ouroboros-6.114.30-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.30/ouroboros-6.114.30-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.30/Ouroboros-6.114.30-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.30/Ouroboros-6.114.30-linux-x86_64.tar.gz
...
| 6.114.26 | 2026-09-12 | **fix(memory): the injected task narrative no longer promises a resume offset it cannot be given.** Fourth site of the same class: `continuation_narrative` truncated with the default single-record surface, so an over-long narrative ended "continue with op='read' offset=N" — unactionable by construction, because its only reader (`main_context_authority._narrative_from_engram`) INJECTS the text into a prompt as `authored_root_summary`: the model never made the call and holds no `observation_id`, and `op='read'` refuses without one. It now uses `_SURFACE_NARRATIVE`: "the whole record via op='search' on this task id, then op='read'" — no `offset=`, and the note still stays inside the caller's bound. The default surface stays the single-record one, as the other callers need; the injection contract (summary_id, `authored_root_summary`, origin, coverage, text bound) is asserted unchanged. Gate: the touched continuation/engram/context suites passed with no failures (junitxml); the full suite was not run for this change. |
