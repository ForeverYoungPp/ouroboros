...
[![Version 6.114.20](https://img.shields.io/badge/version-6.114.20-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.114.20/Ouroboros-6.114.20.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.114.20/Ouroboros-6.114.20-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.114.20/ouroboros_6.114.20_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.20/ouroboros-6.114.20-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.20/ouroboros-6.114.20-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.20/Ouroboros-6.114.20-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.20/Ouroboros-6.114.20-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 6.114.20 | 2026-09-12 | **fix(identity): identity is LOCAL-ONLY — the Engram mirror is retired, and the key path itself is closed (owner ruling, S1 reversed).** The owner ruled that identity is fixed, stable content with no search scenario and no Engram consumer, so nothing may depend on an identity record existing remotely. Removed: the mirror emit in `_update_identity` (`ouroboros/tools/control.py`, the `memory_action` with `identity="identity:manifest"`, `type="identity"`, `scope="global"`, `document=True`) and the `engram_mirror` receipt field it fed in `identity_journal.jsonl`; the local write, journaling and provenance are untouched and nothing in the tool reaches the network. Closed at the sink, because the reflection path could still emit one: `_emit` now refuses any `identity_update_candidate` action and any record addressed at `identity` or `identity:*` with the typed, disclosed reason `identity_local_only` — closing the exact hole the old namespaced-key comment described — and the `IDENTITY_CANDIDATE_MARKER` that only made such a record honest, plus its branch, are gone with it. `prompts/SYSTEM.md:703` no longer claims an Engram record for these tools, and `docs/ARCHITECTURE.md:2696` records the reversal instead of the mirror. Retracted in the live store: observation 177 (`topic_key identity:manifest`, project `ouroboros`), soft delete, `{'hard_delete': False, 'id': 177, 'status': 'deleted'}`. Tests: the mirror suite is replaced by `tests/test_identity_local_only.py` (local write + journal, no store write, P1 project refusal) and the sink candidate test is re-pinned to the refusal; the scratchpad mirror is deliberately out of scope. Gate: the touched identity, engram-sink and governance-doc suites passed with no failures (junitxml); the full suite was not run for this change. |
| 6.114.19 | 2026-09-12 | **fix(chat): the markdown live frame carries `sender_identity` — one background message rendered as two chat bubbles ("Ouroboros" + "🧠 Background") until a refresh.** `_send_markdown` (`supervisor/message_bus.py:978`) accepted the field from v6.114.3 on and never forwarded it to `bridge.send_message`, so every markdown frame — which is exactly what `send_user_message` emits, for the BG lane and the foreground alike — reached the SPA identity-less while its durable row kept the field. The SPA keys a row on its identity (`web/modules/chat_activity.js:643`), so the live bubble could not dedupe against the same durable row replayed from `/api/chat/history`: one message, two bubbles, and a refresh — which rebuilds from history alone — collapsed it to one. The fix is producer-side (one kwarg); NO client-side merge was added, because the client's dedupe was already correct and a second, content-shaped merge would have weakened the identity discrimination v6.114.3 exists to provide. Regression: `test_send_with_budget_live_frame_carries_sender_identity` fails pre-fix (`None != 'background'`) and pins BOTH send branches; the web suite is unchanged at **844/844** (the SPA needed no change). |
| 6.114.18 | 2026-09-12 | **fix(identity): the bounded chat read is now the lane's canonical reader — its prefix omission is DISCLOSED, not a completeness veto (owner policy).** The BG identity gate treated every recent-chat coverage gap as a veto, so `generation_prefix_unscanned` — produced whenever a chat generation exceeds `_AUTOMATIC_CHAT_TAIL_BYTES` (512 KiB, `memory.py:22`) — made `update_identity` abstain indefinitely and left the Engram identity channel empty. `read_unconsolidated_chat`'s unknown flag is now SPLIT (`memory.py:886-894`) instead of folding in `bounded_prefix` — which also covers tail-row omissions: a `generation_prefix_unscanned` gap alone is disclosed through its own coverage entry, while a `generation_tail_rows_unscanned`, every other non-prefix gap kind, and `omitted_generations` (rotated generations) still report unknown. `consciousness.py:1527-1535` blocks on every gap whose kind is not `generation_prefix_unscanned` (row-count, unreadable, unknown kinds — fail-closed), plus `omitted_matching_rows > 0` and the now-narrower unknown flag; the abstain envelope and gate (`consciousness.py:1655-1671`) are untouched. Tests: two BG policy cases and the memory-layer split asserted in three directions; the adjacent pinned expectations are unchanged under the split (`tests/test_context.py:1186`, `:1213`, `:1305` all still `is True`), never loosened. Gate: the touched suites (tests/test_durable_learning_completeness.py + tests/test_context.py) 99 passed / 0 failed via junitxml; the full suite was skipped by owner directive. Honest cost, accepted by the ruling: identity may be authored from a view missing the unscanned prefix (measured live: 273,678 B of logs/chat.jsonl). |
| 6.114.17 | 2026-09-12 | **fix(memory): the governance-map retrieval hint named the one handle that measurably fails.** The low-mode map told a reader to fetch a mirrored chunk by searching its TITLE while the same map, a line earlier, printed the entry's line range and its identity. Measured against the live store: `Govdoc: ARCHITECTURE.md § Delegated subagents … [chunk 123/197]` ranked **17th of 20** on its full title and its heading phrase (`delegated subagents`) missed the top 20 under **every** `match_mode` — the server caps `/search` at 20 hits and the leading title tokens are shared by all 312 records — while its LINE RANGE (`1529`, `1529 1575`) and its identity tail (`architecture:123`, `chunk 123/197`) each ranked **FIRST**. The clause now sends a reader to the line range or the identity, says plainly that the heading words are a weak handle, and keeps `op="read"` on the returned `observation_id`. The probe that motivated it also settled the parameter semantics, read-only against the running server: `/search` **honours** `match_mode` (`all`\|`any`, validated loudly — `and`/`or`/`nonsense`/`ALL`/`"any "` each return 400 naming the two legal values — and the default IS `all`/AND, so the recall seam's OR-matching claim holds); `type=` is **silently ignored**; `sort=` is silently ignored on `/observations/recent` and accepted only as the single value `created_at:desc` on the `/observations` alias, so the recency window is strictly `created_at DESC` and a re-upserted old record never returns to the newest slots — id 122, `review_verdict`, `revision_count 49`, `updated_at 2026-09-11 23:55:45`, still ranks position 319 of 376 under `updated_at DESC`; the displacement of the pre-existing types out of the 50-row window is therefore PERMANENT for those records, and a digest recovers only when a NEW record of that type is created. The intended path was then exercised end-to-end (title-search → `observation_id` → read): three chunks came back WHOLE — 761 / 3,692 / 3,782 chars, no `…[truncated]` marker, `Next:` breadcrumb intact, and the stored body byte-identical to `_sanitize_document` of the record regenerated from the docs on disk. |
| 6.114.16 | 2026-09-12 | **feat(memory): mirror the four governance docs into Engram and make the low-mode maps name the record per entry.** The docs were already readable and already resident in part; what was missing was a MEMORY of them. `prompts/SYSTEM.md`, `BIBLE.md`, `docs/ARCHITECTURE.md` and `docs/DEVELOPMENT.md` are now written to Engram as **312 records** (26 / 16 / 197 / 73), each one returned WHOLESALE by a single `engram op="read"`: the chunker is sized to `MAX_READ_CONTENT_CHARS = 4_000` (`tools/engram.py`), NOT to the sink's 16,000-char document cap, because a record over the read bound comes back truncated and reads exactly like a complete one. Chunking is fence-aware on the same heading definition the map uses, breaks at `##` once a chunk holds 1,500 chars, closes on a paragraph rather than mid-sentence, and splits the five ARCHITECTURE lines longer than the cap (up to 8,642 chars) at spaces into BALANCED parts — the bodies stay an in-order, byte-conserving partition of the doc (pinned by test). Identity is `knowledge:govdoc:<doc>:<ordinal>`, an ORDINAL rather than a start line because several parts of one split line would otherwise upsert over each other; re-running the writer replaces a record instead of duplicating it. Records carry `type="govdoc"`, so the generic knowledge digest can never return a chunk (the type filter is the client-side comparison in `engram_read.py`) — the 45 existing knowledge records are neither rewritten nor re-emitted. The cost is DISCLOSED, not hidden: the recency feed takes no type filter, so the mirror's rows own the newest slots of the bounded `MAX_WINDOW = 50` window and the knowledge DIGEST count falls; the records remain reachable through their primary typed search, and the writer prints the displacement (with a one-record survival witness) before and after. Linkage: `generate_doc_nav_map` gained an opt-in `engram_slug`, so a linked map adds one retrieval sentence plus `· record knowledge:govdoc:<doc>:<n>` per entry, while a map built without it is byte-identical to before — the four existing callers are unchanged. In low mode DEVELOPMENT's not-inlined branch now renders that linked map instead of a bare `docs/DEVELOPMENT.md` name (its mode-independent gate is untouched), and the ARCHITECTURE low branch links the same way. Tier-0 (SYSTEM/BIBLE always full), the ARCH max/low switch, `engram_sink.py`, `engram_read.py`, `engram_client.py`, `tools/knowledge.py`, `tools/engram.py` and the release carriers are untouched. The one-shot writer `.ouroboros/engram-packet/govdoc_ingest.py` is dry-run by default with `--go`, `--limit`, `--only`, `--check` and `--batch-size`, fails closed on any record that would truncate, batch-stages at the 20-emit cap, and writes `govdoc_manifest.json` as an offline drift witness. The ingest itself has NOT been run. |
