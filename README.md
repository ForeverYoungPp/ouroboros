...
[![Version 6.114.22](https://img.shields.io/badge/version-6.114.22-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.114.22/Ouroboros-6.114.22.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.114.22/Ouroboros-6.114.22-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.114.22/ouroboros_6.114.22_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.22/ouroboros-6.114.22-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.22/ouroboros-6.114.22-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.22/Ouroboros-6.114.22-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.22/Ouroboros-6.114.22-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 6.114.22 | 2026-09-12 | **fix(memory): the boot dialogue carry-over looks each block up by identity, not in the flooded recency window.** The mirror check read `client.recent(scope="global", limit=100)` and kept only `dialogue_summary` rows — but `GET /observations/recent` ignores `scope` server-side, so it answers with the newest rows of the PROJECT. After the govdoc backfill owned that window, the dialogue rows (created before every one of those records, and the project's oldest) could never appear in it again: every worker boot concluded "nothing is mirrored" and re-upserted the whole set, visible live as `updated_at` churn while `created_at` stayed frozen. `_mirrored_dialogue_keys` now performs one bounded exact search per locally held block (`type=dialogue_summary`, matched on the normalised `topic_key`), so no other record's volume can displace the check; the `None` => do-nothing contract is unchanged, so an unreachable store, a failing lookup or an unreadable local file still push nothing, while a record the store genuinely lost is still found missing and re-mirrored. The window and its bound are gone together. Gate: the touched engram, dialogue and crash suites passed with no failures (junitxml); the full suite was not run for this change. |
