...
[![Version 6.114.43](https://img.shields.io/badge/version-6.114.43-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.114.43/Ouroboros-6.114.43.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.114.43/Ouroboros-6.114.43-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.114.43/ouroboros_6.114.43_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.43/ouroboros-6.114.43-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.43/ouroboros-6.114.43-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.43/Ouroboros-6.114.43-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.43/Ouroboros-6.114.43-linux-x86_64.tar.gz
...
| 6.114.39 | 2026-09-13 | **fix(usage): the ledger's cold read resumes from a durable watermark instead of re-validating the whole append-only file.** `state/usage_attempts.jsonl` is unbounded, and the per-process warm cache made only the steady state incremental: a cold read — a restart, a cache-slot eviction, or a read failure — still re-parsed and re-validated every row under the monetary flock. `state/usage_attempts.watermark.json` now records the position a read last VALIDATED (inode/device/byte offset/row count plus per-attempt states), written under the ledger's own flock and only AFTER a successful validate. A cold read validates just the bytes appended since and parses the prefix for its rows, returning exactly what a full replay returns; an absent, stale, mismatched or corrupt watermark falls back to the full read, which owns quarantine. It is a read accelerator only — the ledger stays the sole spend authority — and it does not shrink the file. |
