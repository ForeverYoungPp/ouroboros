...
[![Version 6.114.42](https://img.shields.io/badge/version-6.114.42-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.114.42/Ouroboros-6.114.42.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.114.42/Ouroboros-6.114.42-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.114.42/ouroboros_6.114.42_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.42/ouroboros-6.114.42-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.42/ouroboros-6.114.42-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.42/Ouroboros-6.114.42-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.42/Ouroboros-6.114.42-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 6.114.42 | 2026-09-13 | **fix(review): the retrieving reviewer lanes produce verdicts again.** Three measured defects in one lane — a gate that costs real money and returned nothing. (1) **The advisory inlined the touched-file pack into the reviewer it tells to read it.** `build_touched_file_pack` bounds each FILE (`_FILE_SIZE_LIMIT`) and nothing bounds the TOTAL: over the last six real commits the pack measured 938,483-1,147,342 chars with an EMPTY omission list, making every advisory prompt 972,901-1,200,667 — while `role_requirements` ordered the critic to "Read the FULL CONTENT of every changed file with read_file". It was told to re-read ~1M chars it already held; the episode overran its transcript bound, and once that bound was widened it spent its whole round budget re-reading. The retrieving delivery now sends a retrieval recipe (`evidence_by_retrieval`, driven by `include_repo_diff` at the one production call site) naming `vcs_diff`/`read_file`; the staged diff stays inlined because it is the bounded subject (972,901 -> 34,946 chars on the same commits). (2) **The native episode had no forcing function.** Measured with the cap raised to 40 it spent 40/40 rounds across 68 tool calls and still returned no verdict, and on another run it stopped at round 11 with an EMPTY body — so triad (0 of 3 responded), scope and advisory all produced nothing for an inspection already paid for. The LAST permitted round now withdraws the tools (`_FORCED_FINAL_NOTICE`) so the episode delivers the verdict it was paid for. (3) **That deliverable turn's budget was consumed entirely by hidden reasoning** — `completion_tokens == max_tokens` (8192), `finish_reason='length'`, zero content — the same class `OUROBOROS_SAFETY_MAX_TOKENS` exists for. The deliverable turn is now floored at the review surface's own output budget (`native_episode_answer_budget`) while the inspection rounds keep the configured one; an operator who lowered `OUROBOROS_REVIEW_MAX_TOKENS` keeps the behaviour they asked for. Live receipt: the same episode that failed closed twice now returns a 15,072-char verdict (finish='stop', 23,496 completion tokens), and its first finding was a genuinely stale `docs/ARCHITECTURE.md` row in this very diff. Also moves `tests/test_ephemeral_lock_hardening.py` to the serial lane (real threads x process-global supervisor state: it was the entire parallel-pass failure set blocking the gate). |
...
| 6.114.38 | 2026-09-13 | **feat(context): the background lane's knowledge index carries the store's topics without losing the archive's summaries.** The consciousness lane renders `memory/knowledge/index-full.md`, the PRE-SWITCH archive: it froze when the canonical local write was retired, so no lane rendering it alone could NAME a topic written since, while the store-side ledger knew them all. The knowledge section is now the UNION of the two homes — `knowledge_index_union_body`, built from the same rows resolver and entry parser `knowledge_list` uses — so every archive entry keeps its per-topic summary, every store-only topic is named exactly once, and a topic in both appears once with its summary. Only the store half carries the budget (the archive half is frozen at the switch and already resident); an overflow drops the oldest rows and NAMES how many with the full store index pointed at. The resident main-chat projection is unchanged. |

Older releases are preserved in Git tags and GitHub releases. Older 6.x rows (including 6.114.37, 6.114.29, 6.110.1, 6.108.1, 6.106.0, 6.101.1, 6.97.2, 6.105.0, 6.97.1, 6.97.0, 6.96.1, 6.96.0, 6.95.0, 6.94.0, 6.93.0, 6.92.1, 6.92.0, 6.91.1, 6.90.3, 6.91.0, 6.90.2, 6.90.0, 6.87.5, 6.87.4, 6.87.3, 6.87.2, 6.84.0, 6.87.1, 6.83.0, 6.86.1, 6.81.1, 6.76.0, 6.75.0, 6.74.5, 6.74.4, 6.74.1, 6.74.0, 6.73.2, 6.73.1, 6.73.0, 6.72.0, 6.71.2, 6.71.1, 6.71.0, 6.70.0, 6.69.0, 6.68.0, 6.67.0, 6.66.0, 6.65.4, 6.65.3, 6.65.2, 6.65.1, 6.65.0, 6.64.3, 6.64.2, 6.64.1, 6.64.0, 6.63.0, 6.62.0, 6.61.4, 6.61.3, 6.61.1, 6.61.0, 6.60.0, 6.59.0, 6.58.0, 6.57.0, 6.56.0, 6.55.0, 6.54.4, 6.54.2, 6.54.1, 6.54.0, 6.53.4, 6.53.0, 6.51.0), the 5.2.0 through 5.33.0-rc.6 rows, and former `4.0.0` rows are rolled off to respect the P9 changelog cap; their full bodies remain at their git tags.
