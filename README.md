...
[![Version 6.114.37](https://img.shields.io/badge/version-6.114.37-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.114.37/Ouroboros-6.114.37.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.114.37/Ouroboros-6.114.37-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.114.37/ouroboros_6.114.37_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.37/ouroboros-6.114.37-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.37/ouroboros-6.114.37-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.37/Ouroboros-6.114.37-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.37/Ouroboros-6.114.37-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 6.114.37 | 2026-09-13 | **fix(health): the `budget drift` row stops rendering OK from an absent measurement.** It read `state_data.get("budget_drift_alert")` and, on anything falsy, printed "OK: budget drift within tolerance" — but the writer deliberately SUPPRESSES the comparison on a non-final tracked side (`supervisor/state.py:570-575`: "a quarantined ledger tail makes the tracked side non-final; a confident percentage would be dishonest. Comparison is suppressed, not zeroed") and pairs that suppression with `budget_drift_pct=None, alert=False`. The row now distinguishes its three real states: an alert renders the drift with both sides (and tolerates a null percentage instead of crashing into the catch-all), a SUPPRESSED comparison renders `INFO: budget drift UNKNOWN` with the basis named and "Absent is not within tolerance", and a measured comparison renders OK WITH its number. The sibling rows were checked in the same pass: the cache-hit-rate row already renders nothing when the rate is unknown, the identity-freshness row renders nothing when the file is absent, and the scratchpad/size/high-cost/version rows all name a measured value — so the drift row was the one true unknowable-OK. Gate: the context and budget-drift suites passed with no failures (junitxml), including three new pins that fail if the unknowable-OK returns; the full suite was not run, per the owner's standing directive. |
| 6.114.36 | 2026-09-13 | **feat(context): stable-first section order for the prompt-cache horizon, and a real SIZE bound on `## Recent chat`.** Two changes on the mainchat prompt surface. (1) The dynamic block opened with the live scratchpad, so the cacheable prefix ended there; per-call sections (health, scratchpad, recall, drive state, runtime, recent chat and the log tails, presence) now render LAST and the stable or low-frequency ones (registry digest, installed skills, knowledge index, task-contract discipline, backlog digest, review continuity) first. Nothing was added, removed or reworded — order only — and the measured horizon moved from 89.8% to 90.4% of the system message (982,310 to 996,156 chars). (2) `## Recent chat` had only the ENTRY cap (`MAX_RECENT_CHAT_TAIL`), so its worst case was a size: at the measured ~3 KB/row average a full tail renders ~1.5 MB. It now has `RECENT_CHAT_BUDGET_CHARS`, sized ~20% above today's ordinary render so an ordinary turn is byte-identical, with the largest fitting row limit found by bisection and the omitted rows NAMED (count + budget + the `chat_history` and `read_file` pointers), never a silent clip. Gate: the context suite passed with no failures (junitxml), including two order pins re-pinned to the new contract; the full suite was not run, per the owner's standing directive. |
