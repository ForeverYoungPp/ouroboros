...
[![Version 6.114.44](https://img.shields.io/badge/version-6.114.44-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.114.44/Ouroboros-6.114.44.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.114.44/Ouroboros-6.114.44-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.114.44/ouroboros_6.114.44_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.44/ouroboros-6.114.44-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.44/ouroboros-6.114.44-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.44/Ouroboros-6.114.44-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.114.44/Ouroboros-6.114.44-linux-x86_64.tar.gz
...
| 6.114.40 | 2026-09-13 | **fix(context): the knowledge index joins the volatile tail (prompt-cache horizon).** A sweep of the cacheable prefix found the index's rows carry `— updated <date>` and a spooled/refused status (`tools/knowledge.py:215`), so ONE topic write moved the section — and every stable byte after it was then lost to the cache. It now renders first among the volatile sections, so the stable ones (registry, skills, task-contract discipline, backlog, review continuity) all precede the horizon. The dates STAY: BIBLE P1 is relocate-never-delete, and the freshness is what a reader needs. Measured on the projected system message: the first changeable byte moved from 987,554 to 992,776 chars (+5,222; 90.0% of 1,102,833 in max mode). Also verified in the same sweep: the only release-version token in the corpus is the ARCHITECTURE title, and it is RELOCATED to the trailing turn in both modes (max and low build their own projections); the health row's `version sync` token rides the volatile tail and is LEFT because the row IS the check; BIBLE's philosophy version and block[1]'s config fingerprint are LEFT as stable. New pins: a version string must not stay in the cacheable prefix, relocation must MOVE the bytes and leave its pointer, and the ordering property now counts the index as volatile. Gate: the context and prefix suites passed with no failures (junitxml); the full suite was not run, per the owner's standing directive. |
